...
V25_PATH = HERE / "stop03_2_candidate_queues_from_db_safe_v25_0_20260711.py"
V25_CONFIG = HERE / "stop03_2_high_value_policy_v25.json"
FINALIZER_PATH = HERE / "finalize_stop03_2_v25.py"
MIGRATION_PATH = HERE / "20260711_stop03_2_candidate_queue_v25.sql"
if (SCRIPT_DIR / V25_PATH.name).exists():
    V25_PATH = SCRIPT_DIR / V25_PATH.name
    V25_CONFIG = PROJECT / "configs" / "stop03_2_high_value_policy_v25.json"
    FINALIZER_PATH = SCRIPT_DIR / "finalize_stop03_2_v25.py"
    MIGRATION_PATH = PROJECT / "migrations" / "20260711_stop03_2_candidate_queue_v25.sql"
...
v25 = load(V25_PATH, "stop03_2_v25")
v24 = load(
    SCRIPT_DIR / "stop03_2_candidate_queues_from_db_safe_v24_0_20260711.py",
    "stop03_2_v24_for_regression",
)


def cfg():
    return v25.load_config(V25_CONFIG)[0]


def test_v25_formal_policy_identity_is_frozen_candidate():
    config = cfg()
    assert config["policy_version"] == "stop03_2_generic_high_value_policy_v25"
    assert config["policy_status"] in {"FROZEN_CANDIDATE", "FROZEN"}
    assert "rpreplay" in config["screen_recording_path_markers"]
...
def select_images(frames: list[dict], inputs: dict):
    return v25.select_v25_image_candidates(
...
def test_empty_label_sets_cannot_alone_make_timelapse_pair_similar():
    ...
    evidence = v25.timelapse_pair_similarity(left, right, cfg())
...
def video_frame(path: str = "normal/video.mov") -> dict:
    frame = image_frame("video_canonical", "video_source")
    frame.update({
        "media_type": "video", "source_relative_path": path,
...
def test_v25_normal_video_semantics_match_v24_fixture_including_coverage_contract():
    ...
    v25_result = v25.select_candidates([video_frame()], {"video_source": raw_video_frames()}, cfg(), "run", "dry-run", hashes, lineage, empty_inputs())
    v24_cfg = v24.load_config(PROJECT / "configs" / "stop03_2_high_value_policy_v24.json")[0]
    v24_result = v24.select_candidates([video_frame()], {"video_source": raw_video_frames()}, v24_cfg, "run", "dry-run", hashes, lineage, empty_inputs())
    assert v25.video_semantic_set(v25_result["q_rows"]) == v25.video_semantic_set(v24_result["q_rows"])
    assert v25.anchor_indices(13, 6) == v24.anchor_indices(13, 6)
    assert v25.anchor_intervals([6, 0, 12], 13, 3) == v24.anchor_intervals([6, 0, 12], 13, 3)


def test_rpreplay_is_case_insensitive_screen_recording_marker():
    assert v25.is_screen_recording("captures/RPReplay_Final123.MP4", cfg())
    assert v25.is_screen_recording("captures/rpreplay_final123.mp4", cfg())


def test_rpreplay_group_is_removed_from_qwen_and_normal_coverage():
    hashes = {"script_sha256": "s", "config_sha256": "c", "rule_document_sha256": "r"}
    lineage = {"central_dedup_run_id": "d", "yoloe_run_id": "y", "openclip_run_id": "o"}
    result = v25.select_candidates(
        [video_frame("captures/RPReplay_Final123.MP4")],
        {"video_source": raw_video_frames()}, cfg(), "run", "dry-run",
        hashes, lineage, empty_inputs(),
    )
    assert result["q_rows"] == []
    assert result["stats"]["screen_recording_video_group_count"] == 1
    assert result["stats"]["normal_video_group_count"] == 0
...
def test_video_regression_report_passes_for_equal_semantic_sets(tmp_path: Path):
    ...
    rpreplay = {**row, "source_content_id": "screen", "visual_unit_id": "screen_v",
                "canonical_visual_unit_id": "screen_v", "source_relative_path": "RPReplay_Final.MP4"}
    baseline.write_text(
        __import__("json").dumps(row) + "\n" + __import__("json").dumps(rpreplay) + "\n",
        encoding="utf-8",
    )
    report = v25.compare_v24_non_screen_video_semantics([row], baseline)
    assert report["status"] == "PASS"
    assert report["video_logic_changed"] is False
    assert report["excluded_rpreplay_source_count"] == 1


def test_candidate_script_never_alters_schema_and_commit_requires_frozen():
    text = V25_PATH.read_text(encoding="utf-8")
    assert "ALTER TABLE" not in text
    assert 'summary["policy_status"] != "FROZEN"' in text


def test_explicit_migration_and_finalizer_cover_schema_backup_readback_and_idempotency():
    migration = MIGRATION_PATH.read_text(encoding="utf-8")
    finalizer = FINALIZER_PATH.read_text(encoding="utf-8")
    assert "ALTER TABLE stop03_2_candidate_queue_items ADD COLUMN candidate_role TEXT" in migration
    assert "ALTER TABLE stop03_2_candidate_queue_items ADD COLUMN rule_document_sha256 TEXT" in migration
    for token in (
        "A_candidate_dry_run", "B_frozen_dry_run", "C_database_backup",
        "C_commit_run_1", "D_commit_run_2", "restore_database",
        "db_readback_status", "idempotency_status",
    ):
