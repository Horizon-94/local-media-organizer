...
- 保留逐视频原子断点、worker 恢复、中央数据库、强指纹、历史素材库切换与只读搜索。
- 新增本地视频人声链路：VAD、可选降噪、Whisper 转写、时间戳、文本向量与搜索融合。
- 音频中间 WAV 在数据库提交后删除，只保留转写、时间范围、来源关系和向量。
- 搜索素材类型新增“音频（人声转写）”，结果明确显示转写原文、命中时间和命中原因。
- 修复低相关音频证据造成空分数组崩溃的问题；回车键可直接开始搜索。
- 支持多个历史索引库切换；人物名称、标签、合并与撤销保存在各库本地覆盖层。
- App 内含脚本和六套 Python 运行环境，模型、用户数据库、素材与本机配置保持外置。

完整发行说明见 [1.2.0 Release Notes](docs/releases/1.2.0.md)。

## 下载和使用

普通用户优先从 GitHub Releases 下载由 Horizon-94 发布的 macOS DMG。
DMG 内含应用和 Python 运行环境，但**不含任何第三方模型**。
官方 1.2.0 通用 DMG 仅面向 Apple Silicon（M 系列芯片），不提供 Intel、
...
1.2.0 **不把所有电脑固定成开发机的 32 GB 配置**。首次打开和新建任务时，
