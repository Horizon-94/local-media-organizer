...
from .editorial_assistant import build_editorial_board, export_editorial_manifest, export_editorial_timeline
from .editorial_session import import_manifest_session
from .editorial_favorites import editorial_favorites, editorial_search_candidate
from .editorial_preview import resolve_editorial_preview
...
def build_parser() -> argparse.ArgumentParser:
    ...
    editorial = subparsers.add_parser("editorial-board")
    editorial.add_argument("--script-file", type=Path, required=True)
    editorial.add_argument("--guide-file", type=Path, action="append")
    editorial.add_argument("--beat-id")
    editorial.add_argument("--reserved-file", type=Path)
    editorial.add_argument("--saved-beats-file", type=Path)
    editorial.add_argument(
        "--track", choices=("documentary", "short_video"), default="documentary",
    )
    editorial_timeline = subparsers.add_parser("editorial-timeline")
    editorial_timeline.add_argument("--request-file", type=Path, required=True)
    editorial_timeline.add_argument("--output", type=Path, required=True)
    editorial_manifest = subparsers.add_parser("editorial-manifest")
    editorial_manifest.add_argument("--request-file", type=Path, required=True)
    editorial_manifest.add_argument("--output", type=Path, required=True)
    editorial_import = subparsers.add_parser("editorial-import-manifest")
    editorial_import.add_argument("--input", type=Path, required=True)
    editorial_favorite = subparsers.add_parser("editorial-favorites")
    editorial_favorite.add_argument("--board-database", type=Path, required=True)
    editorial_favorite.add_argument("--source-content-id", default="")
    editorial_favorite.add_argument("--offset", type=int, default=0)
    editorial_search = subparsers.add_parser("editorial-search-candidate")
    editorial_search.add_argument("--board-database", type=Path, required=True)
    editorial_search.add_argument("--source-content-id", required=True)
    editorial_search.add_argument("--visual-unit-id", required=True)
    editorial_preview = subparsers.add_parser("editorial-preview")
    editorial_preview.add_argument("--candidate-id", required=True)
    editorial_preview.add_argument("--source-content-id", required=True)
    editorial_preview.add_argument("--board-database", type=Path, required=True)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        elif args.command == "editorial-board":
            if repository is None:
                raise RuntimeError("请先新建或连接一个素材库")
            report = build_editorial_board(
                repository.db_path, args.script_file, args.track, args.guide_file,
                target_beat_id=args.beat_id,
                reserved_visuals=json.loads(args.reserved_file.read_text(encoding="utf-8")) if args.reserved_file else None,
                bound_guidance=json.loads(args.saved_beats_file.read_text(encoding="utf-8")) if args.saved_beats_file else None,
            )
        elif args.command == "editorial-favorites":
            if repository is None:
                raise RuntimeError("请先连接选片工程对应的素材库")
            report = editorial_favorites(repository.db_path, args.board_database, args.source_content_id, args.offset)
        elif args.command == "editorial-search-candidate":
            if repository is None:
                raise RuntimeError("请先连接选片工程对应的素材库")
            report = editorial_search_candidate(repository.db_path, args.board_database, args.source_content_id, args.visual_unit_id)
        elif args.command == "editorial-import-manifest":
            if repository is None:
                raise RuntimeError("请先连接原选片素材库")
            report = import_manifest_session(repository.db_path, args.input)
        elif args.command == "editorial-preview":
            if repository is None:
                raise RuntimeError("请先新建或连接一个素材库")
            report = resolve_editorial_preview(
                repository.db_path, args.candidate_id, args.source_content_id, args.board_database,
            )
        elif args.command == "editorial-timeline":
            if repository is None:
                raise RuntimeError("请先新建或连接一个素材库")
            report = export_editorial_timeline(
                repository.db_path, args.request_file, args.output,
            )
        elif args.command == "editorial-manifest":
            if repository is None:
                raise RuntimeError("请先新建或连接一个素材库")
            report = export_editorial_manifest(
                repository.db_path, args.request_file, args.output,
            )
