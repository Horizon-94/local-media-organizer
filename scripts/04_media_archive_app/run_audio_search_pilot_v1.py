...
def main() -> int:
    ...
    audio_streams = [row for row in probe_payload.get("streams", []) if row.get("codec_type") == "audio"]
    if not audio_streams:
        ...
        result = {
            ...
            "source_video": str(video), "source_stat": source_stat_contract(video),
            "processing_config": {
    ...
    if args.deep_filter_executable and args.deep_filter_model:
        ...
        run([
            ...
            "-i", str(video), "-vn", "-ac", "1", "-ar", "48000",
            "-c:a", "pcm_s16le", str(raw_48k),
        ])
        try:
