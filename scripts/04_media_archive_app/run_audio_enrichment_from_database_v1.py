...
def source_rows(db: Path, limit: int) -> list[sqlite3.Row]:
    with sqlite3.connect(db, timeout=30.0) as con:
        ...
        if "is_deleted_or_missing" not in columns:
            query = (
                "SELECT source_content_id,absolute_path,relative_path FROM source_assets "
                "WHERE media_type='video' ORDER BY source_content_id"
            )
...
def main() -> int:
    ...
    runtime_paths = [
        args.embedding_python, args.audio_pilot_script,
    ...
    for row in videos:
                ...
                con.execute(
                    """INSERT INTO audio_processing_items(
                       ...
                       ON CONFLICT(source_content_id) DO UPDATE SET
                           source_path=excluded.source_path,config_sha256=excluded.config_sha256,
                           status='failed',last_error_code='SOURCE_OFFLINE',
                           last_error_message=excluded.last_error_message,
                           updated_at=CURRENT_TIMESTAMP""",
                    (row["source_content_id"], str(path), config_sha, "原始视频不存在或素材盘未连接"),
                )
                con.commit()
            continue
        stat = path.stat()
        with sqlite3.connect(db, timeout=30.0) as con:
            prior = con.execute(
                "SELECT source_size_bytes,source_mtime_ns,config_sha256,status "
                "FROM audio_processing_items WHERE source_content_id=?",
                (row["source_content_id"],),
            ).fetchone()
            unchanged = bool(prior and prior[:3] == (stat.st_size, stat.st_mtime_ns, config_sha))
            ...
            con.execute(
                ...
                (row["source_content_id"], str(path), stat.st_size, stat.st_mtime_ns, config_sha),
            )
            con.commit()
    with sqlite3.connect(db, timeout=30.0) as con:
        queue = [row for row in con.execute(
            """SELECT source_content_id,source_path FROM audio_processing_items
               ...
               ORDER BY source_content_id""",
            (config_sha,),
        ).fetchall() if str(row[0]) in selected_source_ids]
    total = len(videos)
    already = max(0, total - len(queue))
    success = already
    failed = 0
    skipped = already
    ...
    def submit_item(
        ...
        with sqlite3.connect(db, timeout=30.0) as con:
            ...
            con.commit()
        pilot_command = [
            sys.executable, str(args.audio_pilot_script),
        ...
        while running:
            ...
            for future in done:
                ...
                try:
                    ...
                    with sqlite3.connect(db, timeout=30.0) as con:
                        con.execute(
                            """UPDATE audio_processing_items SET status='failed',last_error_code=?,
    ...
    with sqlite3.connect(db, timeout=30.0) as con:
        ...
        final_status = "success" if failed == 0 and vector_count == segment_count else "failed"
        con.execute(
        ...
        con.commit()
    summary = {
        "contract": CONTRACT, "status": final_status.upper(), "run_id": run_id,
    ...
    return 0 if summary["status"] == "SUCCESS" and integrity == "ok" and not foreign_keys else 1
