...
SCRIPT_VERSION='stop03_2_candidate_queues_from_db_safe_v21_0_20260710_101500'
POLICY_VERSION='stop03_2_generic_high_value_rules_v21_full_video_coverage_then_dedup_then_v14_supplement_20260710'
...
DEFAULT_OUT=TEST_OUTPUT_ROOT/'stop03-2-candidate-queues-db-safe-v21_0_20260710_101500_full'
...
def content_grid_signature(path, cols=16, rows=5):
    """Read derived visual file only. Return luma grid signature and basic visual stats."""
    ...
    try:
        with Image.open(p) as im:
            im = im.convert('L').resize((cols, rows))
        ...
        std = var ** 0.5
        # simple edge/structure score based on neighboring grid differences
        diffs = []
        for y in range(rows):
            for x in range(cols):
                i = y * cols + x
                ...
                if y + 1 < rows:
                    diffs.append(abs(vals[i] - vals[i+cols]))
        structure = sum(diffs) / max(1, len(diffs))
        # center mass: more contrast/detail in center area than edges
        center_vals = []
        edge_vals = []
        for y in range(rows):
            for x in range(cols):
                ...
                if 4 <= x <= 11 and 1 <= y <= 3:
                    ...
                    edge_vals.append(v)
        cmean = sum(center_vals)/max(1,len(center_vals))
        emean = sum(edge_vals)/max(1,len(edge_vals))
        sig = {
            'cols': cols, 'rows': rows, 'vals': vals,
            'mean': mean, 'std': std, 'structure': structure,
            'center_delta': abs(cmean - emean),
        }
        _GRID_CACHE[path] = sig
        return sig
    except Exception:
        _GRID_CACHE[path] = None
        return None

def grid_distance(sig_a, sig_b):
...
def content_score_for_frame(f, labs):
    """No model. Derived frame only. Prefer frames with actual visual structure and center subject."""
    p = f.get('visual_file') or f.get('derived_path') or ''
    sig = content_grid_signature(p)
    score = 0.0
    reasons = []
    if sig:
        # reject very flat / low detail frames
        score += min(sig['structure'] / 18.0, 2.2)
        score += min(sig['std'] / 35.0, 1.8)
        if sig['center_delta'] >= 10:
            score += 0.4
            reasons.append(f'grid_center_delta:{sig["center_delta"]:.1f}')
        reasons.append(f'grid_structure:{sig["structure"]:.1f}')
        reasons.append(f'grid_std:{sig["std"]:.1f}')
    # label support still matters, but does not dominate coverage.
    label_score, label_reasons = base_score(labs)
    score += min(label_score * 0.7, 4.0)
    reasons.extend(label_reasons[:8])
    if not labs:
        reasons.append('no_yoloe_label_content_grid_only')
    return score, reasons, sig

def choose_best_near_anchor(frames, anchor_idx, labels, radius=2):
    """For each V17 anchor, choose the best nearby frame, not blindly the exact anchor."""
    n = len(frames)
    candidates = []
    for off in range(0, radius + 1):
        for j in ([anchor_idx] if off == 0 else [anchor_idx - off, anchor_idx + off]):
            if 0 <= j < n:
                f = frames[j]
                if is_black(f) or is_tail(f, frames):
                    continue
                labs = labels.get(f['visual_unit_id'], [])
                sc, rr, sig = content_score_for_frame(f, labs)
                # mild penalty for moving away from anchor
                sc -= off * 0.18
                candidates.append((sc, j, rr, sig))
    if not candidates:
        return None
    candidates.sort(key=lambda x: (-x[0], abs(x[1]-anchor_idx), x[1]))
    return candidates[0]

def v21_coverage_dedup(chosen_rows, labels, min_gap_ms=18000, grid_threshold=9.0, label_sim_threshold=0.70):
    """Dedup V17 coverage before V14 supplement. This is the important V20 ordering."""
    ...
    for sid, rows in by_source.items():
        ...
        for r in rows:
            ...
            sim = jacc(a,b)
            sig_a = content_grid_signature(prev.get('visual_file') or prev.get('derived_path') or '')
            sig_b = content_grid_signature(r.get('visual_file') or r.get('derived_path') or '')
            gd = grid_distance(sig_a, sig_b)
            same_by_grid = (gd is not None and gd <= float(grid_threshold) and gap < int(min_gap_ms))
            same_by_label = (sim >= float(label_sim_threshold) and gap < int(min_gap_ms))
            if same_by_grid or same_by_label:
                # keep higher content-aware score
                def score_row(x):
                    labs = labels.get(x.get('visual_unit_id'), [])
                    sc, _, _ = content_score_for_frame(x, labs)
                    try: sc += float(x.get('candidate_score') or 0) * 0.15
                    except Exception: pass
                    return sc
                if score_row(r) > score_row(prev):
                    dropped.append(prev); kept[-1] = r
                else:
                    dropped.append(r)
            else:
                kept.append(r)
...
def run(args):
    ...
    for sid,fs in by_video.items():
        ...
        if is_screen(path):
            ...
            for f in chosen:
                ...
                if overlap: stats['v21_v17_v14_overlap_count']+=1
            ...
            for vr in v14_high_by_source.get(sid,[]):
                ...
                if not f or is_black(f) or is_tail(f, sorted_frames):
                    stats['v21_v14_supplement_reject_bad_or_tail_count']+=1
                    continue
                if min_gap_ms(f, selected_video_frames) < int(args.v14_supplement_min_gap_ms):
                    stats['v21_v14_supplement_reject_near_coverage_count']+=1
                    continue
                ...
                selected_video_frames.append(f); selected_vus.add(vu); added+=1; stats['v21_v14_supplement_added_count']+=1
    ...
    stats['v21_final_video_dedup_drop_count'] = len(final_dropped_rows)
    stats['v21_final_video_dedup_before_count'] = q_before_final_dedup
    stats['v21_final_video_dedup_after_count'] = len(q)
    ...
    summary={'validation_status':'PASS' if blackq==0 and blacko==0 and screenq==0 else 'FAIL','policy_version':POLICY_VERSION,'script_version':SCRIPT_VERSION,'run_id':run_id,'input_visual_units':len(rows),'input_video_visual_units':sum(1 for r in rows if r.get('media_type')=='video'),'input_image_visual_units':sum(1 for r in rows if r.get('media_type')=='image'),'qwenvl_total_count':len(q),'qwen_video_frame_count':sum(1 for r in q if r['high_value_category'] in ('video_coverage_keyframe','video_coverage_high_signal_overlap','video_high_signal_supplement')),'qwen_manual_seed_count':sum(1 for r in q if r['high_value_category']=='manual_finder_tag_image_seed'),'qwen_timelapse_count':sum(1 for r in q if r['high_value_category']=='timelapse_candidate'),'qwen_image_yoloe_count':sum(1 for r in q if r['high_value_category']=='image_generic_visual_signal_candidate'),'ocr_total_count':len(o),'qwen_category_counts':dict(Counter(r['high_value_category'] for r in q)),'ocr_media_type_counts':dict(Counter(r.get('media_type') for r in o)),'black_leak_into_qwenvl_count':blackq,'black_leak_into_ocr_count':blacko,'screen_recording_qwenvl_leak_count':screenq,'screen_recording_qwenvl_disabled_group_count':stats['screen_recording_qwenvl_disabled_group_count'],'screen_capture_video_frame_input_count':len(o),'normal_video_ocr_default_excluded_count':stats['normal_video_ocr_weak_excluded_count'],'normal_video_ocr_added_count':stats['normal_video_ocr_added_count'],'normal_video_ocr_cap_excluded_count':stats['normal_video_ocr_cap_excluded_count'],'normal_video_ocr_near_duplicate_excluded_count':stats['normal_video_ocr_near_duplicate_excluded_count'],'tail_suppressed_signal_count':stats['tail_suppressed_signal_count'],'tail_replacement_count':stats['tail_replacement_count'],'black_replacement_count':stats['black_replacement_count'],'same_shot_dedup_drop_count':stats['same_shot_dedup_drop_count'],'v21_anchor_local_best_shift_count':stats['v21_anchor_local_best_shift_count'],'v21_coverage_dedup_drop_count':stats['v21_coverage_dedup_drop_count'],'v21_anchor_drop_no_candidate_count':stats['v21_anchor_drop_no_candidate_count'],'v21_anchor_total_count':stats['v21_anchor_total_count'],'v21_min_one_refill_count':stats['v21_min_one_refill_count'],'v21_min_one_refill_failed_count':stats['v21_min_one_refill_failed_count'],'v21_v17_v14_overlap_count':stats['v21_v17_v14_overlap_count'],'v21_v14_supplement_added_count':stats['v21_v14_supplement_added_count'],'v21_v14_supplement_reject_near_coverage_count':stats['v21_v14_supplement_reject_near_coverage_count'],'v21_v14_supplement_reject_bad_or_tail_count':stats['v21_v14_supplement_reject_bad_or_tail_count'],'v21_final_video_dedup_drop_count':stats['v21_final_video_dedup_drop_count'],'v21_final_video_dedup_before_count':stats['v21_final_video_dedup_before_count'],'v21_final_video_dedup_after_count':stats['v21_final_video_dedup_after_count'],'video_source_group_count':len(by_video),'video_selected_group_count':sum(1 for r in video_budget if int(r['selected_count'])>0),'video_stride':args.video_stride,'settings':{'selection_mode':'v21_full_per_video_coverage_then_dedup_then_v14_supplement_selective_ocr_db_only','video_stride':args.video_stride,'v14_out':str(args.v14_out),'v14_supplement_min_gap_ms':args.v14_supplement_min_gap_ms,'final_video_dedup_min_gap_ms':args.final_video_dedup_min_gap_ms,'final_video_dedup_label_sim_threshold':args.final_video_dedup_label_sim_threshold,'v21_coverage_grid':'16x5_luma_grid','v21_coverage_policy':'all_non_screen_video_groups_full_pass_center_out_stride_then_coverage_dedup_then_v14_supplement','normal_video_ocr_cap':args.normal_video_ocr_cap,'normal_video_ocr_min_score':args.normal_video_ocr_min_score,'normal_video_ocr_min_gap_ms':args.normal_video_ocr_min_gap_ms,'image_yolo_threshold':args.image_yolo_threshold},'safety':pre['safety'],'runtime_preflight':pre,'outputs':{'qwenvl_csv':str(outm/'qwenvl_high_value_candidate_queue.csv'),'ocr_csv':str(outm/'ocr_trigger_candidate_queue.csv'),'summary_json':str(outr/'stop03_2_candidate_summary.json'),'video_budget_report_csv':str(outr/'video_budget_report.csv')},'vector_similarity_policy':'not_used_in_v20_unless_vector_payload_available; current pipeline uses content_grid_signature_16x5_plus_yoloe_labels', 'model_rerun':{'yoloe':False,'openclip':False,'qwen_vl':False,'ocr':False}}
    (outr/'stop03_2_candidate_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
    (outr/'stop03_2_candidate_summary.md').write_text('\n'.join([f'- **{k}**: `{summary.get(k)}`' for k in ['validation_status','qwenvl_total_count','qwen_video_frame_count','ocr_total_count','screen_recording_qwenvl_leak_count','black_leak_into_qwenvl_count','tail_suppressed_signal_count','same_shot_dedup_drop_count','v21_v17_v14_overlap_count','v21_v14_supplement_added_count']]),encoding='utf-8')
    c.execute('''INSERT OR REPLACE INTO model_runs (run_id,stage,model_name,model_path,script_version,script_path,input_count,output_count,status,started_at,finished_at,error_message) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)''',(run_id,'stop03_2_candidate_queues','rule_based_db_v21_full_video_coverage_v14_supplement_no_model','',SCRIPT_VERSION,__file__,len(rows),len(q)+len(o),'done',now(),now(),''))
