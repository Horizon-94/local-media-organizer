...
SCRIPT_VERSION='stop03_2_v21_contact_sheet_20260710_102000'
DEFAULT_DB=Path('/Users/yourname/Documents/AI-Local/media-archive-clean/media_archive.sqlite')
DEFAULT_V21_OUT=Path('/Users/yourname/Documents/AI-Local/test-output/stop03-2-candidate-queues-db-safe-v21_0_20260710_101500_full')
DEFAULT_OUT_DIR=Path('/Users/yourname/Documents/AI-Local/test-output/stop03-2-v21-video-frame-contact-sheet')
...
def main():
    ...
    ap.add_argument('--v21-out',default=str(DEFAULT_V21_OUT))
    ap.add_argument('--out-dir',default=str(DEFAULT_OUT_DIR))
    args=ap.parse_args()
    db=Path(args.db); v21=Path(args.v21_out); outdir=Path(args.out_dir); outdir.mkdir(parents=True,exist_ok=True)
    c=conn(db); frames=load_frames(c); labs=label_counts(c)
    q=read_csv(v21/'manifests/qwenvl_high_value_candidate_queue.csv')
    o=read_csv(v21/'manifests/ocr_trigger_candidate_queue.csv')
    ...
    parts=["<!doctype html><meta charset='utf-8'><title>V21 Video Frame Contact Sheet</title><style>"+css+"</style>",'<h1>V21：V17 覆盖主层 + V14 高信号补充</h1>']
    ...
    html_path=outdir/'v21_video_frame_contact_sheet.html'; html_path.write_text('\n'.join(parts),encoding='utf-8')
    csv_path=outdir/'v21_video_group_summary.csv'
    with csv_path.open('w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=list(summary[0].keys()) if summary else []); w.writeheader(); w.writerows(summary)
    result={'script_version':SCRIPT_VERSION,'db':str(db),'v21_out':str(v21),'video_group_count':len(by),'video_frame_count':len(frames),'output_html':str(html_path),'output_group_summary_csv':str(csv_path)}
