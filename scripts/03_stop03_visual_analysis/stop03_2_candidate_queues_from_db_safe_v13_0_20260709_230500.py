...
# -*- coding: utf-8 -*-
"""
Stop03-2 Candidate Queues from DB - safe V13.0 DB-only timelapse + video time coverage + strict screen-recording OCR + screen-recording Qwen suppression + tail-frame suppression.
...
# -----------------------------------------------------------------------------
SCRIPT_VERSION = "stop03_2_candidate_queues_from_db_safe_v13_0_20260709_230500"
POLICY_VERSION = "stop03_2_generic_high_value_rules_v9_v13_0_strict_screen_ocr_no_screen_qwen_no_tail_20260709"
...
DEFAULT_OUT = TEST_OUTPUT_ROOT / "stop03-2-candidate-queues-db-safe-v13_0_20260709_230500"
...
# V13 strict OCR source gate. User feedback: normal videos had many false OCR
...
def generic_video_group_guard(max_time_ms: int, frame_count: int) -> int:
    ...
    V13 opens the V11 video gate one more step. The goal is not a fixed quota,
    ...
    for r in rows:
        ...
        if r.get("manual_seed"):
            ...
            c.update({
                ...
                "video_frame_rank": "",
                "video_candidate_budget": "reason_based_segment_guard_v13_no_screen_qwen_no_tail_not_fixed_target",
    ...
    for gid, group0 in sorted(video_groups.items()):
        ...
        if SCREEN_RECORDING_QWEN_DISABLED and screen_capture_group:
            video_reports.append({
                ...
                "video_candidate_budget": "screen_recording_qwenvl_disabled_ocr_only_v13",
...
def select_ocr(rows: Sequence[Dict[str, Any]], *, top_global: int) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    # V13 user policy: OCR is strict by default. Still images do not enter OCR.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        summary = {
            ...
            "video_ocr_candidate_source_policy": "strict_screen_capture_video_only; still_image_ocr_disabled; normal_camera_video_ocr_disabled_by_default_in_v13",
            ...
            "settings": {
                ...
                "selection_mode": "generic_dynamic_reason_based_v13_0_no_screen_qwen_no_tail_strict_screen_ocr_db_only_timelapse_no_default_cap",
