...
EXPECTED_PYTHON = PROJECT_ROOT.parent / "envs/media-archive-v06-visual/bin/python"
...
def runtime_preflight() -> Dict[str, Any]:
    """Local dependency/runtime audit. Does not install or download anything."""
    install_offline_env()
    report: Dict[str, Any] = {
        "python_executable": sys.executable,
        "expected_python": str(EXPECTED_PYTHON),
        "expected_python_match": str(resolve_path(Path(sys.executable))) == str(resolve_path(EXPECTED_PYTHON)),
        "dependencies": {},
    }
    for mod_name in ["torch", "open_clip", "safetensors", "PIL"]:
        try:
            mod = __import__(mod_name)
            report["dependencies"][mod_name] = {
                "ok": True,
    ...
    missing = [k for k, v in report["dependencies"].items() if not v.get("ok")]
    report["missing_dependencies"] = missing
    return report


def assert_runtime_preflight() -> Dict[str, Any]:
    report = runtime_preflight()
    if report["missing_dependencies"]:
        fail(
            "BLOCKED_MISSING_LOCAL_DEPENDENCIES",
            "Current Python environment lacks required local packages for OpenCLIP embedding. Use media-archive-v06-visual or install only after explicit user approval.",
            json.dumps(report, ensure_ascii=False),
        )
    return report


def clear_existing_embeddings_for_model(conn: sqlite3.Connection, model_name: str, model_path: Path) -> int:
    cur = conn.execute(
        "DELETE FROM embeddings WHERE model_name=? AND model_path=?",
...
def init_worker(model_path: str, model_name: str, device: str) -> None:
    ...
    # open_clip may log "No pretrained weights loaded" at this construction step;
    # that is expected here because we immediately load the local safetensors file below.
    import logging
    ...
    matched_keys = len(set(state.keys()) & set(model_state.keys()))
    if matched_keys < 100:
        raise RuntimeError(
            f"Local OpenCLIP safetensors appears incompatible with {model_name}: "
            f"matched_keys={matched_keys}, state_keys={len(state)}, model_keys={len(model_state)}"
        )
    missing, unexpected = model.load_state_dict(state, strict=False)
    if len(unexpected) > max(20, len(state) // 5):
        raise RuntimeError(
            f"Too many unexpected keys when loading local OpenCLIP safetensors: "
            f"unexpected={len(unexpected)}, missing={len(missing)}, matched_keys={matched_keys}"
        )
...
def run_db_audit(args: argparse.Namespace) -> None:
    ...
    audit["model_exists"] = model_path.exists()
    audit["runtime_preflight"] = runtime
...
def run_main(args: argparse.Namespace) -> None:
    install_offline_env()
    install_network_guard()
    runtime = assert_runtime_preflight()
    ...
    cleared_embedding_rows = 0
    if args.clear_existing_openclip_embeddings:
        cleared_embedding_rows = clear_existing_embeddings_for_model(conn, args.model_name, model_path)
    ...
    print(json.dumps(
        {
            ...
            "limit": args.limit,
            "clear_existing_openclip_embeddings": args.clear_existing_openclip_embeddings,
            "cleared_embedding_rows": cleared_embedding_rows,
...
def main() -> None:
    ...
    ap.add_argument("--clear-existing-openclip-embeddings", action="store_true")
