...
# -*- coding: utf-8 -*-
"""
Stop03-2 Candidate Queues from DB - safe V7.1 generic-rule convergence test.
...
# -----------------------------------------------------------------------------
SCRIPT_VERSION = "stop03_2_candidate_queues_from_db_safe_v7_1_20260709_193500"
POLICY_VERSION = "stop03_2_generic_high_value_rules_v3_convergence_20260709"
...
DEFAULT_OUT = TEST_OUTPUT_ROOT / "stop03-2-candidate-queues-db-safe-v7_1_20260709_193500"
...
GENERIC_VIDEO_SIGNAL_MIN_SCORE = 1.60
...
def generic_video_group_guard(max_time_ms: int, frame_count: int) -> int:
    ...
    The selector is still reason based, but a single source video should not
    explode into dozens of Qwen-VL jobs merely because it is long or because
    weak YOLOE reasons repeat every extracted frame.
    """
...
def select_qwenvl(rows: Sequence[Dict[str, Any]], *, top_global: int, per_video_budget_scale: float, image_yolo_threshold: float) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for gid, group0 in sorted(video_groups.items()):
        ...
        video_reports.append({
            ...
            "overselect_review_flag": bool(len(picked) > guard or len(picked) > 12 or (len(nonblack) >= 8 and len(picked) / max(1, len(nonblack)) > 0.35)),
    ...
    image_pool0 = [r for r in rows if r.get("visual_unit_type") != "video_frame" and str(r.get("visual_unit_id")) not in selected_ids and not r.get("manual_seed")]
    image_pool0 = [r for r in image_pool0 if not r.get("black_rejected") and row_detection_count(r) > 0 and enough_strong_image_signal(r)]

    # Timelapse: group-level representative selection. Do not send all timelapse keyframes.
    timelapse_groups: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    normal_image_pool: List[Dict[str, Any]] = []
    for r in image_pool0:
        if r.get("source_group_kind") == "timelapse":
            timelapse_groups[str(r.get("source_group_id") or r.get("source_content_id") or r.get("derived_id") or r.get("visual_unit_id"))].append(r)
        else:
            normal_image_pool.append(r)

    image_pool: List[Tuple[Dict[str, Any], str]] = []
    for gid, grp in timelapse_groups.items():
        grp = sorted(grp, key=lambda r: (int(r.get("time_position_ms") or 0), -float(r.get("qwen_score") or 0.0), str(r.get("visual_unit_id"))))
        if not grp:
            continue
        # Default one representative. If a sequence is larger and labels change strongly,
        # allow two; this keeps typical 3-frame timelapse groups near one middle/best frame.
        middle = grp[len(grp)//2]
        best = max(grp, key=lambda r: (float(r.get("qwen_score") or 0.0), row_detection_count(r)))
        chosen = [best if float(best.get("qwen_score") or 0.0) > float(middle.get("qwen_score") or 0.0) + 1.0 else middle]
        if len(grp) >= 5:
            labels_first = label_set(grp[0]); labels_last = label_set(grp[-1])
            if jaccard_distance(labels_first, labels_last) >= 0.72:
                second = grp[-1] if chosen[0] is not grp[-1] else grp[0]
    ...
    try:
        ...
        summary = {
            ...
            "settings": {
                ...
                "selection_mode": "generic_dynamic_reason_based_convergence_v7_1_no_default_cap",
