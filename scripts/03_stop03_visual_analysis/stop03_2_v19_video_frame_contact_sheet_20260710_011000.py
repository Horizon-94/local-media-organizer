...
SCRIPT_VERSION='stop03_2_v19_contact_sheet_20260710_011000'
DEFAULT_DB=Path('/Users/yourname/Documents/AI-Local/media-archive-clean/media_archive.sqlite')
DEFAULT_V19_OUT=Path('/Users/yourname/Documents/AI-Local/test-output/stop03-2-candidate-queues-db-safe-v19_0_20260710_010500_full')
DEFAULT_OUT_DIR=Path('/Users/yourname/Documents/AI-Local/test-output/stop03-2-v19-video-frame-contact-sheet')
...
def main():
    ...
    ap.add_argument('--v19-out',default=str(DEFAULT_V19_OUT))
    ap.add_argument('--out-dir',default=str(DEFAULT_OUT_DIR))
    args=ap.parse_args()
    db=Path(args.db); v19=Path(args.v19_out); outdir=Path(args.out_dir); outdir.mkdir(parents=True,exist_ok=True)
    c=conn(db); frames=load_frames(c); labs=label_counts(c)
    q=read_csv(v19/'manifests/qwenvl_high_value_candidate_queue.csv')
    o=read_csv(v19/'manifests/ocr_trigger_candidate_queue.csv')
    ...
    parts=["<!doctype html><meta charset='utf-8'><title>V19 Video Frame Contact Sheet</title><style>"+css+"</style>",'<h1>V19：V17 覆盖主层 + V14 高信号补充</h1>']
    ...
    html_path=outdir/'v19_video_frame_contact_sheet.html'; html_path.write_text('\n'.join(parts),encoding='utf-8')
    csv_path=outdir/'v19_video_group_summary.csv'
    with csv_path.open('w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=list(summary[0].keys()) if summary else []); w.writeheader(); w.writerows(summary)
    result={'script_version':SCRIPT_VERSION,'db':str(db),'v19_out':str(v19),'video_group_count':len(by),'video_frame_count':len(frames),'output_html':str(html_path),'output_group_summary_csv':str(csv_path)}
