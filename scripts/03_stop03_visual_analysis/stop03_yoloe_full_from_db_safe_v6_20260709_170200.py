...
# -*- coding: utf-8 -*-
"""
Stop03 YOLOE full from DB, local-only safe v6 runtime-pinned concurrent + enriched bilingual terms.
...
- Create/update enriched visual_label_terms from local prompt registry for zh/bilingual search/embedding routing.
...
REQUIRED_LOCAL_ASSETS = {
    "db": DEFAULT_DB,
    "model": DEFAULT_MODEL,
    "mobileclip": DEFAULT_MOBILECLIP,
    "registry": DEFAULT_REGISTRY,
    "project_root": PROJECT_ROOT,
    "model_root": MODEL_ROOT,
}
REQUIRED_PYTHON_MODULES = ["torch", "ultralytics", "PIL"]
...
# Worker globals for concurrent mode. Each process loads the local YOLOE model once.
_WORKER_MODEL = None
_WORKER_CLASSES: List[str] = []
_WORKER_MODEL_PATH: Optional[Path] = None
_WORKER_MOBILECLIP_PATH: Optional[Path] = None
_WORKER_DEVICE: str = "mps"
...
def _module_check(module_name: str) -> Dict[str, Any]:
    """Best-effort local dependency check. Does not install or download anything."""
    info: Dict[str, Any] = {"ok": False, "version": "", "error": ""}
    try:
...
def runtime_preflight(db: Path, model: Path, mobileclip: Path, registry: Optional[Path]) -> Dict[str, Any]:
    """Pinned local runtime and asset audit for repeatable offline execution.

    This is intentionally explicit so the script itself documents the expected
    Python environment, local model assets, prompt registry, and dependency set.
    It never installs dependencies, never downloads models, and never opens the network.
    """
    # Set offline env before dependency imports. This is not a download guard by itself;
    # the hard socket guard is still installed before Ultralytics model loading.
    ...
    py_real = py.resolve()
    bundled_app_python = (
        "/Contents/Frameworks/PipelinePython" in str(py_real)
        and "/Python.framework/Versions/" in str(py_real)
    )
    deps = {m: _module_check(m) for m in REQUIRED_PYTHON_MODULES}
    assets: Dict[str, Any] = {}
    local_paths = {
        "db": db,
        "model": model,
        "mobileclip": mobileclip,
        "registry": registry if registry else DEFAULT_REGISTRY,
        "project_root": PROJECT_ROOT,
        "model_root": MODEL_ROOT,
    }
    for name, p in local_paths.items():
        if p is None:
            assets[name] = {"path": "", "exists": False, "size_bytes": None}
        else:
            try:
                assets[name] = {
                    "path": str(p),
                ...
                assets[name] = {"path": str(p), "exists": False, "size_bytes": None, "error": f"{type(e).__name__}: {e}"}

    blockers: List[str] = []
    if py_real != EXPECTED_PYTHON.resolve() and not bundled_app_python:
        blockers.append(f"python_mismatch:expected={EXPECTED_PYTHON};actual={py}")
    for name, p in [("db", db), ("model", model), ("mobileclip", mobileclip), ("registry", registry if registry else DEFAULT_REGISTRY)]:
        if p and not p.exists():
            blockers.append(f"{name}_missing:{p}")
    for m, info in deps.items():
        if not info.get("ok"):
            blockers.append(f"dependency_missing:{m}:{info.get('error','')}")
    try:
        if not str(model.resolve()).startswith(str(MODEL_ROOT.resolve())):
            blockers.append(f"model_outside_model_root:{model.resolve()}")
        if not str(mobileclip.resolve()).startswith(str(MODEL_ROOT.resolve())):
            blockers.append(f"mobileclip_outside_model_root:{mobileclip.resolve()}")
    except Exception:
        pass
    ...
    return {
        ...
        "expected_python_match": py_real == EXPECTED_PYTHON.resolve() or bundled_app_python,
        "bundled_app_python": bundled_app_python,
        "script_version": SCRIPT_VERSION,
        "expected_script_local": str(EXPECTED_SCRIPT_LOCAL),
        "project_root": str(PROJECT_ROOT),
        "model_root": str(MODEL_ROOT),
        "required_local_assets": {k: str(v) for k, v in REQUIRED_LOCAL_ASSETS.items()},
        "assets": assets,
        "dependencies": deps,
        "missing_dependencies": [m for m, info in deps.items() if not info.get("ok")],
        "offline_env": {
...
def enrich_search_terms(label: str, label_zh: str, current_terms: Iterable[Any], query_terms: Iterable[Any]) -> List[str]:
    """Chinese-first search expansion for retrieval/embedding only, not YOLOE prompt expansion."""
    terms: List[str] = []
    # Keep the stable English label and display zh first, then registry/query aliases, then project synonyms.
    for v in [label, label_zh]:
        _append_unique(terms, v)
    for v in current_terms:
        _append_unique(terms, v)
    for v in EXTRA_SYNONYMS.get(label, []):
        _append_unique(terms, v)
    for v in query_terms:
        _append_unique(terms, v)
    # Split common bilingual/compound zh display values like "车 / 小汽车" into separate searchable terms.
    for part in str(label_zh or "").replace("／", "/").split("/"):
        _append_unique(terms, part.strip())
    return terms
...
def build_label_terms_from_registry(registry: Optional[Path], include_b: bool, include_ocr_trigger: bool) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    reverse_query_mappings: Dict[str, List[str]] = {}
    for query, labels in (query_mappings or {}).items():
        for label in labels or []:
            label = str(label or "").strip()
            if not label:
                continue
            reverse_query_mappings.setdefault(label, [])
            _append_unique(reverse_query_mappings[label], query)
    ...
    for label in sorted(terms):
        ...
        search_terms = enrich_search_terms(label, label_zh, t.get("search_terms") or [label], reverse_query_mappings.get(label, []))
...
def ensure_visual_label_terms_table(con: sqlite3.Connection) -> None:
    ...
    """)
    existing_cols = {r[1] for r in con.execute("PRAGMA table_info(visual_label_terms)").fetchall()}
...
def upsert_visual_label_terms(con: sqlite3.Connection, rows: List[Dict[str, Any]]) -> int:
    ...
    cols = [
        ...
        "registry_path", "registry_schema_version", "created_at", "updated_at",
    ]
    q = f"""
...
def worker_init(model_path_s: str, mobileclip_path_s: str, classes: List[str], device: str) -> None:
    """Initialize one local-only YOLOE model per worker process."""
    global _WORKER_MODEL, _WORKER_CLASSES, _WORKER_MODEL_PATH, _WORKER_MOBILECLIP_PATH, _WORKER_DEVICE
    _WORKER_CLASSES = list(classes)
    _WORKER_MODEL_PATH = Path(model_path_s)
    _WORKER_MOBILECLIP_PATH = Path(mobileclip_path_s)
    _WORKER_DEVICE = device
    install_offline_guard(_WORKER_MOBILECLIP_PATH)
    from ultralytics import YOLOE, YOLO  # type: ignore
    patch_ultralytics_local_mobileclip(_WORKER_MOBILECLIP_PATH)
    try:
        model = YOLOE(str(_WORKER_MODEL_PATH))
    except Exception:
        model = YOLO(str(_WORKER_MODEL_PATH))
    patch_ultralytics_local_mobileclip(_WORKER_MOBILECLIP_PATH)
    if hasattr(model, "set_classes"):
        patch_ultralytics_local_mobileclip(_WORKER_MOBILECLIP_PATH)
        if hasattr(model, "get_text_pe"):
            model.set_classes(_WORKER_CLASSES, model.get_text_pe(_WORKER_CLASSES))
        else:
            model.set_classes(_WORKER_CLASSES)
    _WORKER_MODEL = model


def worker_predict_one(task: Tuple[int, int, Dict[str, Any], int, float, str]) -> Dict[str, Any]:
    """Run one image prediction in a worker. Returns detections only; DB writes stay in main process."""
    global _WORKER_MODEL, _WORKER_CLASSES
    idx, total, row, imgsz, conf, device = task
    ...
    try:
        if _WORKER_MODEL is None:
            raise RuntimeError("worker_model_not_initialized")
        if not image_path.exists():
            raise FileNotFoundError(f"visual_file_missing:{image_path}")
        res = _WORKER_MODEL.predict(source=str(image_path), imgsz=imgsz, conf=conf, device=device, verbose=False)
        result = res[0] if isinstance(res, list) else res
        detections = detect_result_to_rows(result, _WORKER_MODEL, _WORKER_CLASSES)
...
def db_audit(db: Path, model: Path, mobileclip: Path, registry: Optional[Path]) -> Dict[str, Any]:
    ...
    return {
        ...
        "runtime_preflight": runtime_preflight(db, model, mobileclip, registry),
        "counts": counts,
        "schemas": schema,
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--concurrency", type=int, default=4, help="Worker process count for image inference. Default 4. Use 1 for serial debug.")
...
def main() -> int:
    ...
    if args.db_audit_only:
        ...
        blockers = ((audit.get("runtime_preflight") or {}).get("blockers") or [])
        return 0 if audit.get("model_exists") and audit.get("mobileclip_exists") and not blockers else 2

    preflight = audit.get("runtime_preflight") or runtime_preflight(db, model_path, mobileclip_path, registry)
    problems: List[str] = []
    problems.extend(preflight.get("blockers") or [])
    ...
    try:
        ...
        summary = {"validation_status": "FAIL", "error": f"ultralytics_import_failed:{type(e).__name__}:{e}", "runtime_preflight": preflight, "offline_report": offline_report}
        write_json(out / "reports" / "stop03_yoloe_full_summary.json", summary)
        print(json.dumps(summary, ensure_ascii=False, indent=2))
        return 2
    ...
    if concurrency <= 1:
        # Serial debug path. Kept close to V3 behavior.
        try:
            patch_ultralytics_local_mobileclip(mobileclip_path)
            try:
                model = YOLOE(str(model_path))  # type: ignore[name-defined]
            except Exception:
                model = YOLO(str(model_path))  # type: ignore[name-defined]
            patch_ultralytics_local_mobileclip(mobileclip_path)
            if hasattr(model, "set_classes"):
                patch_ultralytics_local_mobileclip(mobileclip_path)
                if hasattr(model, "get_text_pe"):
            ...
            summary = {"validation_status": "FAIL", "error": err, "traceback": traceback.format_exc(), "runtime_preflight": preflight, "offline_report": offline_report, "ultralytics_patch_report": ultralytics_patch_report}
            write_json(out / "reports" / "stop03_yoloe_full_summary.json", summary)
            print(json.dumps(summary, ensure_ascii=False, indent=2))
            return 2
        ...
        for idx, r in enumerate(rows, start=1):
            ...
            task_rec = worker_predict_one((idx, len(rows), row, args.imgsz, args.conf, args.device)) if False else None
            one_t0 = time.time()
            rec: Dict[str, Any] = {
                "task_index": idx,
                "task_total": len(rows),
        ...
        # Concurrent path: four independent local-only worker processes by default.
        total = len(rows)
        max_workers = min(concurrency, total) if total else concurrency
        print(f"[concurrency] mode=process workers={max_workers} input_rows={total}", flush=True)
        tasks = [(idx, total, dict(r), args.imgsz, args.conf, args.device) for idx, r in enumerate(rows, start=1)]
        completed = 0
        try:
            with ProcessPoolExecutor(
                max_workers=max_workers,
                initializer=worker_init,
                initargs=(str(model_path), str(mobileclip_path), classes, args.device),
            ) as ex:
                fut_map = {}
                for task in tasks:
                    idx, total_n, row, _imgsz, _conf, _device = task
                    image_path = Path(str(row.get("visual_file") or ""))
                    print(f"[submit {idx}/{total_n}] visual_unit_id={row.get('visual_unit_id')} file={image_path.name}", flush=True)
                    fut_map[ex.submit(worker_predict_one, task)] = task
                for fut in as_completed(fut_map):
                    ...
                    try:
                        ...
                        rec = {
                            ...
                            "source_content_id": row.get("source_content_id"),
                            "derived_id": row.get("derived_id"),
                            "visual_file": str(row.get("visual_file") or ""),
                            "source_relative_path": row.get("source_relative_path"),
                            "time_position_ms": row.get("time_position_ms"),
                            "status": "failed",
                            "detection_count": 0,
                    ...
                    # Main process writes DB to avoid SQLite multi-writer lock collisions.
                    if rec.get("status") == "success":
                        try:
                            ...
                            if detections:
                                positive_units += 1
                            rec["labels"] = "|".join(sorted(set(labels)))
                            rec["detection_count"] = len(detections)
                            print(f"[done {completed}/{total}] task={rec.get('task_index')} visual_unit_id={rec.get('visual_unit_id')} detections={len(detections)} labels={rec.get('labels')}", flush=True)
                        except Exception as e:
                            failed += 1
                            con.rollback()
                            rec["status"] = "failed"
                            rec["error_message"] = ("db_write_failed:" + str(e) + "\n" + traceback.format_exc())[:4000]
                            print(f"[fail {completed}/{total}] task={idx} visual_unit_id={row.get('visual_unit_id')} error=db_write_failed:{str(e)[:260]}", flush=True)
                    else:
                        failed += 1
                        print(f"[fail {completed}/{total}] task={rec.get('task_index')} visual_unit_id={rec.get('visual_unit_id')} error={str(rec.get('error_message') or '')[:300]}", flush=True)
                    rec.pop("detections", None)
                    result_rows.append(rec)
                    if completed % 50 == 0 or completed == total:
                        print(f"progress {completed}/{total} processed={processed} failed={failed} detections={detection_rows}", flush=True)
        except Exception as e:
            failed += max(1, len(rows) - processed)
            err = f"concurrent_executor_failed:{type(e).__name__}:{e}"
            print(f"[fail executor] {err}", flush=True)
            result_rows.append({"status": "failed", "error_message": err + "\n" + traceback.format_exc()})
    ...
    summary = {
        ...
        "settings": {"device": args.device, "imgsz": args.imgsz, "conf": args.conf, "limit": args.limit, "concurrency": concurrency},
        "concurrency_policy": "ProcessPoolExecutor; one local YOLOE model per worker; main process only writes SQLite",
        "counts": {
