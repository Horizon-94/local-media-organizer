...
# -*- coding: utf-8 -*-
"""
Stop03-2 Candidate Queues from DB - safe V6 generic-rule test.
...
# -----------------------------------------------------------------------------
SCRIPT_VERSION = "stop03_2_candidate_queues_from_db_safe_v6_20260709_203800"
POLICY_VERSION = "stop03_2_generic_high_value_rules_v1_20260709"
...
DEFAULT_OUT = TEST_OUTPUT_ROOT / "stop03-2-candidate-queues-db-safe-v6_20260709_203800"
...
def generic_video_frame_signal(row: Dict[str, Any], prev_row: Optional[Dict[str, Any]], next_row: Optional[Dict[str, Any]], idx: int, n: int) -> Tuple[bool, List[str], float]:
    ...
    prev_dist = jaccard_distance(labels, label_set(prev_row or {})) if prev_row else 0.0
    next_dist = jaccard_distance(labels, label_set(next_row or {})) if next_row else 0.0
    ...
    if ocr_score > 0:
        ...
        score_boost += 0.35
    if row_detection_count(row) >= 4 or len(labels) >= 3:
        reasons.append("high_information_jump")
        score_boost += 0.35
    if idx == 0 or idx == n - 1:
        # Boundary is weak; only useful if there is some visual/OCR signal.
        if qscore >= 0.35 or ocr_score > 0:
            reasons.append("video_coverage_boundary")
            score_boost += 0.15
    return bool(reasons), reasons, score_boost
...
def select_qwenvl(rows: Sequence[Dict[str, Any]], *, top_global: int, per_video_budget_scale: float, image_yolo_threshold: float) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for r in rows:
        ...
        if r.get("manual_seed"):
            ...
            c.update({
                ...
                "video_frame_rank": "",
                "video_candidate_budget": "dynamic_segment_rule_no_fixed_budget",
    ...
    for gid, group0 in sorted(video_groups.items()):
        ...
        video_reports.append({
            ...
            "selected_ratio_note": "not_percentage_based",
        })
    ...
    try:
        ...
        summary = {
            ...
            "settings": {
                ...
                "selection_mode": "generic_dynamic_reason_based_not_percentage_not_fixed_target",
