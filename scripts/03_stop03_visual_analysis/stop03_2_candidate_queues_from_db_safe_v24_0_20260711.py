...
SCRIPT_VERSION = "stop03_2_candidate_queues_from_db_safe_v24_0_20260711"
POLICY_VERSION = "stop03_2_generic_high_value_policy_v24"
STAGE = "stop03_2_candidate_queues_v24"
...
DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "stop03_2_high_value_policy_v24.json"
RULE_DOCUMENT = PROJECT_ROOT / "docs" / "pipeline_rules" / "STOP03_2_GENERIC_HIGH_VALUE_RULES_V24.md"
DEFAULT_OUT = TEST_OUTPUT_ROOT / "stop03_2_v24_dry_run"
...
def make_candidate(
    ...
    return {
        "candidate_id": stable_id("cand_v24_", POLICY_VERSION, queue_type, role, frame["visual_unit_id"]),
...
def select_candidates(
    ...
    image_result = select_v24_image_candidates(
    ...
    for decision in decisions.values():
        if not decision["qwen_selected"] and not decision["ocr_selected"]:
            decision["decision_reason_codes"].append("not_selected_by_v24_policy")
    return {
...
def load_v24_image_inputs(con: sqlite3.Connection) -> dict[str, list[dict[str, Any]]]:
    ...
    if missing:
        raise RuntimeError("v24_image_input_tables_missing:" + ",".join(missing))
...
def select_v24_image_candidates(
...
def compare_v23_video_semantics(
    v24_rows: Sequence[Mapping[str, Any]], baseline_path: Path | None,
) -> dict[str, Any]:
    current = video_semantic_set(v24_rows)
    if baseline_path is None:
        return {
            "status": "NOT_REQUESTED", "video_logic_changed": False,
            "video_regression_ready": False, "v24_video_count": len(current),
            "baseline_path": "", "baseline_video_count": None,
            "added_count": None, "removed_count": None,
        }
    baseline_rows = []
    with baseline_path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            try:
                ...
                raise RuntimeError(f"v23_video_baseline_jsonl_invalid:{line_number}:{exc}") from exc
    baseline = video_semantic_set(baseline_rows)
    ...
    return {
        ...
        "baseline_video_count": len(baseline), "v24_video_count": len(current),
...
def build_summary(
    ...
    if video_regression.get("status") != "NOT_REQUESTED":
        gates["v23_video_semantic_regression_pass"] = video_regression.get("status") == "PASS"
    if mode == "dry-run":
    ...
    return {
        ...
        "policy_reason_codes": ["v24_frozen_candidate_pending_review"] if pre["config"]["policy_status"] == "FROZEN_CANDIDATE" else [],
        "policy_version": POLICY_VERSION, "script_version": SCRIPT_VERSION,
        ...
        **signature_stats, **vector_stats, **label_stats,
        ...
        "video_v23_semantic_regression_status": video_regression.get("status"),
...
def write_outputs(out: Path, result: dict[str, Any], summary: dict[str, Any], reverse_rows: list[dict[str, Any]]) -> dict[str, str]:
    ...
    paths = {
        ...
        "video_v23_semantic_regression_json": reports / "video_v23_semantic_regression.json",
    ...
    write_json(paths["video_v23_semantic_regression_json"], result["video_regression"])
...
def ensure_candidate_columns(con: sqlite3.Connection) -> None:
    ...
    for name, sql_type in V24_DB_COLUMNS.items():
...
def commit_candidates(
    ...
    try:
        ...
        columns = [
            ...
            "run_id", "script_version", "created_at", *V24_DB_COLUMNS,
        ]
        ...
        con.execute(
            ...
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                summary["run_id"], STAGE, "rule_based_v24_no_model", "", SCRIPT_VERSION,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone Stop03-2 V24 generic candidate queues")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if pre["technical_status"] != "PASS":
            raise RuntimeError("v24_preflight_failed")
        mode = args.mode
        ...
        try:
            ...
            if video_signature_failures:
                raise RuntimeError("v24_video_derived_signature_failures")
            ...
            image_inputs = load_v24_image_inputs(con)
        finally:
            con.close()
        ...
        result["video_regression"] = compare_v23_video_semantics(result["q_rows"], baseline_path)
        result_repeat["video_regression"] = compare_v23_video_semantics(result_repeat["q_rows"], baseline_path)
        ...
        if summary["technical_status"] != "PASS":
            raise RuntimeError("v24_candidate_technical_gates_failed:" + json_text(summary["automatic_acceptance_gates"]))
