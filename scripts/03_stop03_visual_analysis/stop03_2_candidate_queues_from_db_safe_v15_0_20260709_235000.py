...
# -*- coding: utf-8 -*-
"""
Stop03-2 Candidate Queues from DB - safe V15.0 DB-only timelapse + strict screen OCR + no screen Qwen + stronger tail suppression + same-shot de-dup + human representative preference.
...
# -----------------------------------------------------------------------------
SCRIPT_VERSION = "stop03_2_candidate_queues_from_db_safe_v15_0_20260709_235000"
POLICY_VERSION = "stop03_2_generic_high_value_rules_v11_v15_0_same_shot_dedup_human_representative_no_tail_20260709"
...
DEFAULT_OUT = TEST_OUTPUT_ROOT / "stop03-2-candidate-queues-db-safe-v15_0_20260709_235000"
...
GENERIC_VIDEO_SIGNAL_MIN_SCORE = 1.25
GENERIC_IMAGE_SIGNAL_MIN_SCORE = 4.20
GENERIC_LABEL_CHANGE_THRESHOLD = 0.62
...
# V14 strict OCR source gate. User feedback: normal videos had many false OCR
# positives from weak YOLOE labels such as book/sign/billboard. OCR should be
# sent by default only for screen-recording / screenshot-like sources. Normal
# camera video OCR is disabled unless a future explicit allowlist is added.
STRICT_SCREEN_CAPTURE_PATH_KEYWORDS = [
    "rpreplay", "screenrecording", "screen_recording", "screen-recording",
    "screen recording", "record screen", "recorded screen",
    "录屏", "屏幕录制", "截屏", "截图", "screenshot",
]

# Screen-recording videos should not create Qwen-VL high-value visual frames by
# default. They can still go to OCR.
SCREEN_RECORDING_QWEN_DISABLED = True

# Tail/end frames are often landing frames. They should not be high-value by
# default unless a future manual/explicit exception is added.
TAIL_FRAME_SUPPRESS_RATIO = 0.10
TAIL_FRAME_SUPPRESS_MIN_MS = 6000
TAIL_FRAME_SUPPRESS_MAX_MS = 30000

# V14 human/composition preference. This is still metadata-only: it uses existing
# YOLOE label boxes from the central DB and derived preview dimensions. It does
# not run face recognition, identity recognition, pose models, eye-state models,
# or any online model. The goal is simply to prefer centered/complete human
# frames over edge/cropped/hand-only/tail frames when multiple candidates exist.
COMPOSITION_FACE_CENTER_BONUS = 1.35
COMPOSITION_PERSON_CENTER_BONUS = 1.05
COMPOSITION_OFFCENTER_HUMAN_PENALTY = 0.90
COMPOSITION_HAND_ONLY_PENALTY = 1.40
COMPOSITION_EDGE_CROP_PENALTY = 0.75
COMPOSITION_CENTER_MAX_DIST = 0.36
COMPOSITION_EDGE_MARGIN = 0.045

# V15 post-selection quality pass. It does not run any new model. It compares
# already selected frames against neighboring candidates in the same source video
# using DB timecodes, YOLOE label sets, bbox-derived composition quality, and tail
# position. Goal: avoid selecting near-identical frames or poor human frames when
# a nearby centered/cleaner representative exists.
V15_LOCAL_REPRESENTATIVE_WINDOW_MS = 24000
V15_REDUNDANT_WINDOW_MS = 45000
V15_REDUNDANT_LABEL_DISTANCE_MAX = 0.34
V15_REFILL_MIN_GAP_MS = VIDEO_CANDIDATE_MIN_GAP_MS
...
def load_raw_label_boxes(con: sqlite3.Connection) -> Dict[str, List[Dict[str, Any]]]:
    """Load per-box YOLOE metadata for V14 composition preference.

    Existing load_labels() intentionally aggregates by label for scoring. V14 also
    needs individual boxes to judge center/edge/hand-only patterns. This reads only
    the central SQLite table visual_labels.
    """
    if not table_exists(con, "visual_labels"):
        return {}
    cols = table_columns(con, "visual_labels")
    conf_col = "confidence" if "confidence" in cols else ("score" if "score" in cols else None)
    bbox_col = "bbox" if "bbox" in cols else None
    select_conf = f"COALESCE({conf_col}, 0.0)" if conf_col else "0.0"
    select_bbox = bbox_col if bbox_col else "''"
    sql = f"""
    SELECT visual_unit_id, label, {select_conf} AS confidence, {select_bbox} AS bbox
    FROM visual_labels
    """
    out: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for r in con.execute(sql).fetchall():
...
def load_units(con: sqlite3.Connection) -> List[Dict[str, Any]]:
    ...
    SELECT
      ...
      da.width AS visual_width,
      da.height AS visual_height,
      sa.media_type,
      sa.extension,
    ...
    raw_label_boxes = load_raw_label_boxes(con)
    ...
    for r in rows:
        ...
        r["raw_label_boxes"] = raw_label_boxes.get(vu, [])
...
def path_haystack(row: Dict[str, Any]) -> str:
    return " ".join([
        str(row.get("source_relative_path") or ""),
        str(row.get("original_source_path_at_processing_time") or ""),
        str(row.get("visual_file") or ""),
...
def strict_screen_capture_path_hits(row: Dict[str, Any]) -> List[str]:
    hay = path_haystack(row)
    return sorted({kw for kw in STRICT_SCREEN_CAPTURE_PATH_KEYWORDS if kw.lower() in hay})


def is_screen_capture_source(row: Dict[str, Any]) -> bool:
    return bool(strict_screen_capture_path_hits(row))


def is_tail_video_frame(row: Dict[str, Any], min_time_ms: int, max_time_ms: int) -> bool:
    try:
        t = int(row.get("time_position_ms") or 0)
    except Exception:
        return False
    duration = max(0, int(max_time_ms) - int(min_time_ms))
    if duration <= 30000:
        # For very short clips, the only available representative may naturally
        # be near the end; do not suppress short-video coverage.
        return False
    tail_window = int(max(TAIL_FRAME_SUPPRESS_MIN_MS, min(TAIL_FRAME_SUPPRESS_MAX_MS, duration * TAIL_FRAME_SUPPRESS_RATIO)))
    return t >= int(max_time_ms) - tail_window


def parse_bbox_any(raw: Any, width: Any = None, height: Any = None) -> Optional[Tuple[float, float, float, float]]:
    """Parse YOLOE bbox into normalized x1,y1,x2,y2 if possible."""
    if raw is None:
        return None
    try:
        ...
        if isinstance(obj, dict):
            if all(k in obj for k in ["x1", "y1", "x2", "y2"]):
                vals = [float(obj[k]) for k in ["x1", "y1", "x2", "y2"]]
            ...
            if len(obj) < 4:
                return None
            vals = [float(obj[i]) for i in range(4)]
            # Common alternatives are xyxy or xywh. If it looks like xywh, convert.
            if vals[2] <= vals[0] or vals[3] <= vals[1]:
                vals = [vals[0], vals[1], vals[0] + abs(vals[2]), vals[1] + abs(vals[3])]
        else:
            return None
        x1, y1, x2, y2 = vals[:4]
        maxv = max(abs(x1), abs(y1), abs(x2), abs(y2))
        if maxv > 2.0:
            w = float(width or 0) or max(x1, x2, 1.0)
            h = float(height or 0) or max(y1, y2, 1.0)
            x1, x2 = x1 / w, x2 / w
            y1, y2 = y1 / h, y2 / h
...
def bbox_center_quality(b: Tuple[float, float, float, float]) -> Dict[str, float]:
    x1, y1, x2, y2 = b
    cx = (x1 + x2) / 2.0
    cy = (y1 + y2) / 2.0
    dist = math.sqrt((cx - 0.5) ** 2 + (cy - 0.5) ** 2)
    area = max(0.0, (x2 - x1) * (y2 - y1))
    touches_edge = 1.0 if (x1 <= COMPOSITION_EDGE_MARGIN or y1 <= COMPOSITION_EDGE_MARGIN or x2 >= 1.0 - COMPOSITION_EDGE_MARGIN or y2 >= 1.0 - COMPOSITION_EDGE_MARGIN) else 0.0
    center_score = max(0.0, 1.0 - dist / 0.7072)
    # People/face boxes that are too tiny should not dominate; large but edge-cropped is penalized separately.
    size_score = min(1.0, max(0.0, area / 0.18))
    return {"cx": cx, "cy": cy, "dist": dist, "area": area, "edge": touches_edge, "center_score": center_score, "size_score": size_score}


def human_composition_adjustment(row: Dict[str, Any]) -> Tuple[float, List[str]]:
    """Prefer centered complete human subject frames; demote hand-only/edge-cropped frames.

    Limits: no eye-state/front-face model is used. This cannot truly know whether
    eyes are open. It uses available YOLOE boxes as a safe local proxy: face/person
    centrality, box size, and edge crop.
    """
    raw = row.get("raw_label_boxes") or []
    width = row.get("visual_width")
    height = row.get("visual_height")
    reasons: List[str] = []
    best_face: Optional[Dict[str, float]] = None
    best_person: Optional[Dict[str, float]] = None
    has_handish = False
    for item in raw:
        name = str(item.get("label") or "").lower()
        box = parse_bbox_any(item.get("bbox"), width, height)
        if any(k in name for k in ["hand", "finger", "arm"]):
            has_handish = True
        if box is None:
            continue
        q = bbox_center_quality(box)
        conf = float(item.get("confidence") or 0.0)
        q["conf"] = conf
        rank = q["center_score"] + 0.35 * q["size_score"] + 0.15 * conf - 0.35 * q["edge"]
        q["rank"] = rank
        if "face" in name:
            if best_face is None or rank > best_face["rank"]:
                best_face = q
        if any(k in name for k in ["person", "human", "body"]):
            if best_person is None or rank > best_person["rank"]:
                best_person = q
    adj = 0.0
    subject = best_face or best_person
    if best_face:
        if best_face["dist"] <= COMPOSITION_CENTER_MAX_DIST and best_face["edge"] < 1 and best_face["area"] >= 0.015:
            adj += COMPOSITION_FACE_CENTER_BONUS
            reasons.append(f"composition_prefer_centered_face:dist={best_face['dist']:.2f}:area={best_face['area']:.3f}")
        else:
            adj -= COMPOSITION_OFFCENTER_HUMAN_PENALTY
            reasons.append(f"composition_penalty_face_offcenter_or_cropped:dist={best_face['dist']:.2f}:edge={int(best_face['edge'])}:area={best_face['area']:.3f}")
    elif best_person:
        if best_person["dist"] <= COMPOSITION_CENTER_MAX_DIST and best_person["edge"] < 1 and best_person["area"] >= 0.025:
            adj += COMPOSITION_PERSON_CENTER_BONUS
            reasons.append(f"composition_prefer_centered_person:dist={best_person['dist']:.2f}:area={best_person['area']:.3f}")
        else:
            adj -= COMPOSITION_OFFCENTER_HUMAN_PENALTY
            reasons.append(f"composition_penalty_person_offcenter_or_cropped:dist={best_person['dist']:.2f}:edge={int(best_person['edge'])}:area={best_person['area']:.3f}")
    if subject and subject.get("edge", 0) >= 1:
        adj -= COMPOSITION_EDGE_CROP_PENALTY
    if has_handish and not best_face and not best_person:
        adj -= COMPOSITION_HAND_ONLY_PENALTY
        reasons.append("composition_penalty_hand_or_finger_without_centered_person")
    return adj, reasons
...
def compute_scores(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    comp_adj, comp_reasons = human_composition_adjustment(row)
    if comp_adj:
        q += comp_adj
        q_reasons.extend(comp_reasons)
        row["composition_quality_adjustment"] = round(comp_adj, 6)
        row["composition_quality_reasons"] = comp_reasons
    else:
        row["composition_quality_adjustment"] = 0.0
        row["composition_quality_reasons"] = []
    ...
    row["strict_screen_capture_path_hits"] = strict_screen_capture_path_hits(row)
    row["is_screen_capture_source"] = bool(row["strict_screen_capture_path_hits"])
    if path_hits:
...
def generic_video_group_guard(max_time_ms: int, frame_count: int) -> int:
    ...
    V14 opens the V11 video gate one more step. The goal is not a fixed quota,
    but the expected 30GB video candidate ratio should move from about 11.3%
    toward the user's preferred 12.5%–15% band when real frame evidence
...
def has_centered_human_proxy(row: Dict[str, Any]) -> bool:
    return float(row.get("composition_quality_adjustment") or 0.0) > 0 and any(
        "composition_prefer_centered" in x for x in (row.get("composition_quality_reasons") or [])
    )


def has_bad_human_or_hand_proxy(row: Dict[str, Any]) -> bool:
    return float(row.get("composition_quality_adjustment") or 0.0) < 0 or any(
        ("hand_or_finger" in x or "offcenter" in x or "cropped" in x)
        for x in (row.get("composition_quality_reasons") or [])
    )


def video_representative_quality(row: Dict[str, Any], min_time_ms: int, max_time_ms: int) -> float:
    """V15 local representative score using existing DB metadata only."""
    base = float(row.get("qwen_score") or 0.0)
    comp = float(row.get("composition_quality_adjustment") or 0.0)
    det_bonus = min(0.55, row_detection_count(row) * 0.035)
    score = base + comp * 1.55 + det_bonus
    if has_centered_human_proxy(row):
        score += 0.85
    if has_bad_human_or_hand_proxy(row):
        score -= 1.10
    if is_tail_video_frame(row, min_time_ms, max_time_ms):
        score -= 2.50
    try:
        t = int(row.get("time_position_ms") or 0)
        if t == min_time_ms or t == max_time_ms:
            score -= 0.35
    except Exception:
        pass
    return score
...
def rows_are_redundant_same_shot(a: Dict[str, Any], b: Dict[str, Any]) -> bool:
    try:
        gap = abs(int(a.get("time_position_ms") or 0) - int(b.get("time_position_ms") or 0))
    except Exception:
        gap = 10**12
    if gap > V15_REDUNDANT_WINDOW_MS:
        return False
    la = label_set(a)
    lb = label_set(b)
    if not la and not lb:
        return False
    dist = jaccard_distance(la, lb)
    if dist <= V15_REDUNDANT_LABEL_DISTANCE_MAX:
        return True
    human_a = any(x in la for x in {"person", "face", "human", "people"})
    human_b = any(x in lb for x in {"person", "face", "human", "people"})
    if human_a and human_b and gap <= 30000 and dist <= 0.55:
        return True
    return False


def v15_consolidate_video_picks(
    picked: List[Tuple[Dict[str, Any], List[str], float]],
    signal_rows: List[Tuple[Dict[str, Any], List[str], float]],
    nonblack: Sequence[Dict[str, Any]],
    min_time_ms: int,
    max_time_ms: int,
    guard: int,
) -> Tuple[List[Tuple[Dict[str, Any], List[str], float]], Dict[str, int]]:
    stats = {"local_replacement_count": 0, "same_shot_dedup_drop_count": 0, "refill_count": 0}
    if not picked:
        return picked, stats
    signal_list = list(signal_rows)
    replaced: List[Tuple[Dict[str, Any], List[str], float]] = []
    used_vu: set[str] = set()
    for r, reasons, boost in picked:
        try:
            t = int(r.get("time_position_ms") or 0)
        except Exception:
            t = 0
        current_quality = video_representative_quality(r, min_time_ms, max_time_ms) + float(boost or 0.0)
        local_pool = []
        for rr, rr_reasons, rr_boost in signal_list:
            vu = str(rr.get("visual_unit_id"))
            if vu in used_vu:
                continue
            try:
                tt = int(rr.get("time_position_ms") or 0)
            except Exception:
                tt = 0
            if abs(tt - t) <= V15_LOCAL_REPRESENTATIVE_WINDOW_MS and not is_tail_video_frame(rr, min_time_ms, max_time_ms):
                local_pool.append((rr, rr_reasons, rr_boost))
        if local_pool:
            best = max(local_pool, key=lambda x: video_representative_quality(x[0], min_time_ms, max_time_ms) + float(x[2] or 0.0))
            best_quality = video_representative_quality(best[0], min_time_ms, max_time_ms) + float(best[2] or 0.0)
            should_replace = (
                str(best[0].get("visual_unit_id")) != str(r.get("visual_unit_id"))
                and (best_quality >= current_quality + 0.45 or is_tail_video_frame(r, min_time_ms, max_time_ms) or has_bad_human_or_hand_proxy(r))
            )
            if should_replace:
                stats["local_replacement_count"] += 1
                rr, rr_reasons, rr_boost = best
                replaced.append((rr, list(rr_reasons) + ["v15_local_better_representative_replaced_poor_or_tail_frame"], rr_boost))
                used_vu.add(str(rr.get("visual_unit_id")))
                continue
        replaced.append((r, reasons, boost))
        used_vu.add(str(r.get("visual_unit_id")))
    ordered = sorted(replaced, key=lambda x: int(x[0].get("time_position_ms") or 0))
    kept: List[Tuple[Dict[str, Any], List[str], float]] = []
    for item in ordered:
        r, reasons, boost = item
        redundant_idx = None
        for i, old in enumerate(kept):
            if rows_are_redundant_same_shot(r, old[0]):
                redundant_idx = i
                break
        if redundant_idx is None:
            kept.append(item)
        else:
            old = kept[redundant_idx]
            old_score = video_representative_quality(old[0], min_time_ms, max_time_ms) + float(old[2] or 0.0)
            new_score = video_representative_quality(r, min_time_ms, max_time_ms) + float(boost or 0.0)
            stats["same_shot_dedup_drop_count"] += 1
            if new_score > old_score:
                kept[redundant_idx] = (r, list(reasons) + ["v15_same_shot_kept_better_representative"], boost)
            else:
                old[1].append("v15_same_shot_kept_over_redundant_neighbor")
    kept_vus = {str(x[0].get("visual_unit_id")) for x in kept}
    refill_pool = sorted(
        [x for x in signal_list if str(x[0].get("visual_unit_id")) not in kept_vus and not is_tail_video_frame(x[0], min_time_ms, max_time_ms)],
        key=lambda x: -(video_representative_quality(x[0], min_time_ms, max_time_ms) + float(x[2] or 0.0)),
    )
    for rr, rr_reasons, rr_boost in refill_pool:
        if len(kept) >= guard:
            break
        try:
            t = int(rr.get("time_position_ms") or 0)
        except Exception:
            t = 0
        if any(abs(t - int(k[0].get("time_position_ms") or 0)) < V15_REFILL_MIN_GAP_MS for k in kept):
            continue
        if any(rows_are_redundant_same_shot(rr, k[0]) for k in kept):
            continue
        kept.append((rr, list(rr_reasons) + ["v15_refill_after_same_shot_dedup"], rr_boost))
        stats["refill_count"] += 1
    return sorted(kept, key=lambda x: int(x[0].get("time_position_ms") or 0)), stats
...
def generic_video_frame_signal(row: Dict[str, Any], prev_row: Optional[Dict[str, Any]], next_row: Optional[Dict[str, Any]], idx: int, n: int) -> Tuple[bool, List[str], float]:
    ...
    comp_adj = float(row.get("composition_quality_adjustment") or 0.0)
    comp_reasons = list(row.get("composition_quality_reasons") or [])
    if comp_adj > 0 and any("centered" in x for x in comp_reasons):
        reasons.append("human_composition_preferred")
        strong_reasons.append("human_composition_preferred")
        score_boost += min(0.85, comp_adj * 0.45)
    elif comp_adj < 0:
        reasons.extend(comp_reasons[:2])
    ...
    if label_change >= GENERIC_LABEL_CHANGE_THRESHOLD and qscore >= 1.0:
    ...
    ocr_scene_change = ocr_score > 0 and label_change >= 0.35 and qscore >= 0.8
    ...
    info_jump = False
    if det >= max(prev_det, next_det) + 4 and det >= 6 and distinct >= 2:
        info_jump = True
    if distinct >= max(prev_distinct, next_distinct) + 2 and distinct >= 3:
        info_jump = True
    if det >= 10 and distinct >= 3 and label_change >= 0.35:
        info_jump = True
    if info_jump:
...
def select_qwenvl(rows: Sequence[Dict[str, Any]], *, top_global: int, per_video_budget_scale: float, image_yolo_threshold: float) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for r in rows:
        ...
        if r.get("manual_seed"):
            ...
            c.update({
                ...
                "video_frame_rank": "",
                "video_candidate_budget": "reason_based_segment_guard_v15_same_shot_dedup_human_representative_no_screen_qwen_no_tail_not_fixed_target",
    ...
    for gid, group0 in sorted(video_groups.items()):
        ...
        screen_capture_group = any(is_screen_capture_source(r) for r in group)
        if SCREEN_RECORDING_QWEN_DISABLED and screen_capture_group:
            video_reports.append({
                "source_group_id": gid,
                "frame_count": len(group),
                "nonblack_frame_count": len(raw_nonblack),
                "raw_nonblack_frame_count": len(raw_nonblack),
                "duplicate_visual_unit_same_derived_count": 0,
                "black_rejected_count": len(group) - len(raw_nonblack),
                "missing_effective_time_count": sum(1 for rr in raw_nonblack if int(rr.get("time_position_ms") or -1) < 0),
                "video_candidate_budget": "screen_recording_qwenvl_disabled_ocr_only_v15",
                "video_segment_guard": 0,
                "candidate_signal_count": 0,
                "selected_count": 0,
                "overselect_review_flag": False,
                "selected_ratio_note": "screen_recording_group_skipped_for_qwenvl",
                "v15_local_replacement_count": 0,
                "v15_same_shot_dedup_drop_count": 0,
                "v15_refill_count": 0,
            })
            for r in group:
                vu = str(r.get("visual_unit_id"))
                if vu not in decisions:
                    decisions[vu] = {"q_reject": "screen_recording_qwenvl_disabled_ocr_only", "q_score": r.get("qwen_score", 0.0), "q_reason": ""}
            continue
        ...
        min_time_ms = min((int(r.get("time_position_ms") or 0) for r in nonblack), default=0)
        max_time_ms = max((int(r.get("time_position_ms") or 0) for r in nonblack), default=0)
        signal_rows: List[Tuple[Dict[str, Any], List[str], float]] = []
        tail_suppressed_signal_count = 0
        for i, r in enumerate(nonblack):
            ...
            if ok:
                if is_tail_video_frame(r, min_time_ms, max_time_ms):
                    tail_suppressed_signal_count += 1
                    continue
                signal_rows.append((r, reasons, boost))
        if not signal_rows and nonblack:
            fallback_pool = [r for r in nonblack if not is_tail_video_frame(r, min_time_ms, max_time_ms)] or nonblack
            best = max(fallback_pool, key=lambda r: (float(r.get("qwen_score") or 0.0), row_detection_count(r), -abs(int(r.get("time_position_ms") or 0))))
            signal_rows = [(best, ["video_min_one_best_available_frame"], 0.0)]

        guard = generic_video_group_guard(max_time_ms, len(nonblack))
        picked: List[Tuple[Dict[str, Any], List[str], float]] = []
        ...
        picked, v15_consolidation_stats = v15_consolidate_video_picks(picked, signal_rows, nonblack, min_time_ms, max_time_ms, guard)
        ...
        video_reports.append({
            ...
            "tail_suppressed_signal_count": tail_suppressed_signal_count,
            "screen_capture_group": False,
            ...
            "v15_local_replacement_count": int(v15_consolidation_stats.get("local_replacement_count") or 0),
            "v15_same_shot_dedup_drop_count": int(v15_consolidation_stats.get("same_shot_dedup_drop_count") or 0),
            "v15_refill_count": int(v15_consolidation_stats.get("refill_count") or 0),
        })
...
def select_ocr(rows: Sequence[Dict[str, Any]], *, top_global: int) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    # V14 user policy: OCR is strict by default. Still images do not enter OCR.
    # Normal camera videos do not enter OCR because YOLOE text-like labels produced
    # too many false positives. Only screen-recording / screenshot-like video
    # sources enter OCR by default.
    ocr_rows = [
        r for r in rows
        if r.get("visual_unit_type") == "video_frame"
        and not r.get("black_rejected")
        and bool(is_screen_capture_source(r))
    ...
    for r in ocr_rows:
        ...
        c.update({
            ...
            "known_ocr_like_source_group": bool(is_screen_capture_source(r)),
        })
        out.append(c)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        summary = {
            ...
            "normal_video_ocr_default_excluded_count": sum(1 for r in rows if r.get("visual_unit_type") == "video_frame" and not r.get("black_rejected") and float(r.get("ocr_score") or 0.0) > 0 and not is_screen_capture_source(r)),
            "screen_capture_video_frame_input_count": sum(1 for r in rows if r.get("visual_unit_type") == "video_frame" and is_screen_capture_source(r)),
            "screen_recording_qwenvl_disabled_group_count": sum(1 for vr in video_reports if vr.get("selected_ratio_note") == "screen_recording_group_skipped_for_qwenvl"),
            "tail_suppressed_signal_count": sum(int(vr.get("tail_suppressed_signal_count") or 0) for vr in video_reports),
            "composition_prefer_centered_human_count": sum(1 for r in rows if any("composition_prefer_centered" in x for x in (r.get("composition_quality_reasons") or []))),
            "composition_penalty_human_bad_or_hand_only_count": sum(1 for r in rows if float(r.get("composition_quality_adjustment") or 0.0) < 0),
            "v15_local_replacement_count": sum(int(vr.get("v15_local_replacement_count") or 0) for vr in video_reports),
            "v15_same_shot_dedup_drop_count": sum(int(vr.get("v15_same_shot_dedup_drop_count") or 0) for vr in video_reports),
            "v15_refill_count": sum(int(vr.get("v15_refill_count") or 0) for vr in video_reports),
            "video_ocr_candidate_source_policy": "strict_screen_capture_video_only; still_image_ocr_disabled; normal_camera_video_ocr_disabled_by_default_in_v15",
            ...
            "settings": {
                ...
                "selection_mode": "generic_dynamic_reason_based_v15_0_same_shot_dedup_human_representative_no_screen_qwen_no_tail_strict_screen_ocr_db_only_timelapse_no_default_cap",
