...
DEFAULT_OUT = TEST_OUTPUT_ROOT / "stop03_2_v25_dry_run"
...
def preflight(db: Path, out: Path, config_path: Path) -> dict[str, Any]:
    ...
    required = {
        ...
        "model_runs", "stop03_2_candidate_queue_items", "source_finder_tags",
    ...
    with connect_readonly(db) as con:
        ...
        candidate_missing_columns = missing_candidate_columns(con)
    ...
    return {
        ...
        "candidate_schema_status": "PASS" if not candidate_missing_columns else "MIGRATION_REQUIRED_BEFORE_COMMIT",
        "candidate_schema_missing_columns": candidate_missing_columns,
...
def make_candidate(
    ...
    return {
        "candidate_id": stable_id("cand_v25_", POLICY_VERSION, queue_type, role, frame["visual_unit_id"]),
...
def select_candidates(
    ...
    lineage: Mapping[str, str], image_inputs: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    image_result = select_v25_image_candidates(
        rows, image_inputs, config, run_id, mode, hashes, lineage,
    )
    q_rows.extend(image_result["q_rows"])
    stats.update(image_result["stats"])
    ...
    for decision in decisions.values():
        if not decision["qwen_selected"] and not decision["ocr_selected"]:
            decision["decision_reason_codes"].append("not_selected_by_v25_policy")
    return {
        ...
        "image_selection_audit": image_result["audit"],
        "image_selection_summary": image_result["summary"],
    }


def load_v25_image_inputs(con: sqlite3.Connection) -> dict[str, list[dict[str, Any]]]:
    required = ("source_finder_tags", "step02_image_timelapse_keyframes")
    missing = [name for name in required if not object_exists(con, name)]
    if missing:
        raise RuntimeError("v25_image_input_tables_missing:" + ",".join(missing))
    finder_tags = [dict(row) for row in con.execute(
        "SELECT tag_id,source_content_id,tag_raw,tag_name,tag_color FROM source_finder_tags ORDER BY tag_id"
    )]
    source_assets = [dict(row) for row in con.execute(
        "SELECT source_content_id,relative_path,file_name,extension,media_type,is_deleted_or_missing FROM source_assets ORDER BY source_content_id"
    )]
    visual_mappings = [dict(row) for row in con.execute(
        """SELECT vu.visual_unit_id,vu.source_content_id,vi.canonical_visual_unit_id,
                  vi.visual_duplicate_group_id,vi.identity_status
           FROM visual_units vu JOIN visual_identity vi ON vi.visual_unit_id=vu.visual_unit_id
           ORDER BY vu.source_content_id,vu.visual_unit_id"""
    )]
    timelapse_rows = [dict(row) for row in con.execute(
        """SELECT visual_unit_id,sequence_id,representative_position,source_relative_path,
                  visual_file,parent_source_content_id
           FROM step02_image_timelapse_keyframes
           ORDER BY sequence_id,representative_position,visual_unit_id"""
    )]
    return {
        "finder_tags": finder_tags,
        "source_assets": source_assets,
        "visual_mappings": visual_mappings,
        "timelapse_rows": timelapse_rows,
    }


def normalized_sidecar_key(source: Mapping[str, Any]) -> tuple[str, str]:
    relative = str(source.get("relative_path") or "").replace("\\", "/")
    parent = Path(relative).parent.as_posix().casefold()
    file_name = str(source.get("file_name") or Path(relative).name)
    return parent, Path(file_name).stem.casefold()


def timelapse_pair_similarity(
    left: Mapping[str, Any], right: Mapping[str, Any], config: Mapping[str, Any]
) -> dict[str, Any]:
    exact = bool(left.get("derived_sha256")) and left.get("derived_sha256") == right.get("derived_sha256")
    vector = cosine(left.get("vector"), right.get("vector"))
    ...
    vector_close = vector is not None and vector >= safe_float(config["dedup_vector_cosine_threshold"])
    grid_close = (
        mad is not None and corr is not None
        and mad <= safe_float(config["dedup_grid_mad_threshold"])
        and corr >= safe_float(config["dedup_grid_correlation_threshold"])
    )
    label_close = labels is not None and labels >= safe_float(config["dedup_label_jaccard_threshold"])
    similar = exact or ((vector_close or grid_close) and (label_close or (vector_close and grid_close)))
    return {
        "similar": similar, "exact_derived_identity": exact,
        "vector_cosine": vector, "vector_close": vector_close,
        "grid_mad": mad, "grid_correlation": corr, "grid_close": grid_close,
        "label_jaccard": labels, "nonempty_label_close": label_close,
    }


def select_timelapse_positions(
    positions: Mapping[str, Mapping[str, Any]], config: Mapping[str, Any]
) -> tuple[list[str], str, dict[str, Any]]:
    available = [position for position in ("first", "middle", "last") if position in positions]
    evidence: dict[str, Any] = {}
    if not available:
        return [], "no_valid_position", evidence
    if len(available) == 1:
        return available, "single_valid_position", evidence
    if set(available) == {"first", "middle", "last"}:
        evidence["first_middle"] = timelapse_pair_similarity(positions["first"], positions["middle"], config)
        evidence["middle_last"] = timelapse_pair_similarity(positions["middle"], positions["last"], config)
        first_middle = bool(evidence["first_middle"]["similar"])
        middle_last = bool(evidence["middle_last"]["similar"])
        if first_middle and middle_last:
            return ["middle"], "both_segments_similar", evidence
        if first_middle:
            return ["middle", "last"], "last_segment_changed", evidence
        if middle_last:
            return ["first", "middle"], "first_segment_changed", evidence
        return ["first", "middle", "last"], "both_segments_changed", evidence
    left, right = available[0], available[1]
    evidence[f"{left}_{right}"] = timelapse_pair_similarity(positions[left], positions[right], config)
    if evidence[f"{left}_{right}"]["similar"]:
        representative = "middle" if "middle" in available else left
        return [representative], "remaining_positions_similar", evidence
    return available, "remaining_positions_changed", evidence


def select_v25_image_candidates(
    rows: Sequence[MutableMapping[str, Any]], image_inputs: Mapping[str, Any],
    config: Mapping[str, Any], run_id: str, mode: str, hashes: Mapping[str, str],
    lineage: Mapping[str, str],
) -> dict[str, Any]:
    image_by_id = {
        str(row["visual_unit_id"]): row for row in rows if row.get("media_type") == "image"
    }
    sources = {str(row["source_content_id"]): row for row in image_inputs["source_assets"]}
    mappings_by_source: dict[str, list[dict[str, Any]]] = defaultdict(list)
    mapping_by_visual: dict[str, dict[str, Any]] = {}
    for mapping in image_inputs["visual_mappings"]:
        mappings_by_source[str(mapping["source_content_id"])].append(mapping)
        mapping_by_visual[str(mapping["visual_unit_id"])] = mapping

    audit: list[dict[str, Any]] = []
    stats: Counter[str] = Counter()
    finder_by_canonical: dict[str, list[dict[str, Any]]] = defaultdict(list)
    xmp_by_canonical: dict[str, list[dict[str, Any]]] = defaultdict(list)
    timelapse_by_canonical: dict[str, list[dict[str, Any]]] = defaultdict(list)

    finder_tags = list(image_inputs["finder_tags"])
    stats["finder_tag_input_row_count"] = len(finder_tags)
    finder_sources = {str(row.get("source_content_id") or "") for row in finder_tags}
    stats["finder_tag_unique_source_count"] = len(finder_sources)
    finder_image_sources: set[str] = set()
    finder_non_image_sources: set[str] = set()
    finder_missing_sources: set[str] = set()
    for tag in finder_tags:
        source_id = str(tag.get("source_content_id") or "")
        tag_nonempty = any(str(tag.get(key) or "").strip() for key in ("tag_raw", "tag_name", "tag_color"))
        source = sources.get(source_id)
        base = {
            "audit_type": "finder_tag", "tag_id": tag.get("tag_id"),
            "tag_raw": tag.get("tag_raw"), "tag_name": tag.get("tag_name"),
            "tag_color": tag.get("tag_color"), "tagged_source_content_id": source_id,
        }
        if not tag_nonempty or source is None or safe_int(source.get("is_deleted_or_missing"), 0) != 0:
            finder_missing_sources.add(source_id)
            audit.append({**base, "selection_status": "missing_or_empty_tag_source"})
            continue
        if str(source.get("media_type") or "") != "image":
            finder_non_image_sources.add(source_id)
            audit.append({**base, "selection_status": "non_image_source"})
            continue
        finder_image_sources.add(source_id)
        mappings = mappings_by_source.get(source_id, [])
        if not mappings:
            audit.append({**base, "selection_status": "missing_visual_mapping"})
        for mapping in mappings:
            stats["finder_tag_mapped_visual_count"] += 1
            canonical_id = str(mapping.get("canonical_visual_unit_id") or "")
            detail = {
                **base, "tagged_original_visual_unit_id": mapping.get("visual_unit_id"),
                "canonical_visual_unit_id": canonical_id,
                "duplicate_group_id": mapping.get("visual_duplicate_group_id") or "",
            }
            if canonical_id not in image_by_id:
                audit.append({**detail, "selection_status": "canonical_visual_unavailable"})
                continue
            finder_by_canonical[canonical_id].append(detail)
            audit.append({**detail, "selection_status": "mapped_canonical"})
    stats["finder_tag_image_source_count"] = len(finder_image_sources)
    stats["finder_tag_non_image_source_count"] = len(finder_non_image_sources)
    stats["finder_tag_missing_source_count"] = len(finder_missing_sources)
    stats["finder_tag_canonical_unique_count"] = len(finder_by_canonical)
    stats["finder_tag_canonical_collapse_count"] = max(
        0, stats["finder_tag_mapped_visual_count"] - len(finder_by_canonical)
    )

    active_sources = [
        source for source in sources.values()
        if safe_int(source.get("is_deleted_or_missing"), 0) == 0
    ]
    xmp_sources = [
        source for source in active_sources
        if str(source.get("extension") or "").strip().lower().lstrip(".") == "xmp"
    ]
    image_sources = [source for source in active_sources if source.get("media_type") == "image"]
    images_by_key: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for source in image_sources:
        images_by_key[normalized_sidecar_key(source)].append(source)
    matched_image_sources: set[str] = set()
    xmp_missing_visual_sources: set[str] = set()
    stats["xmp_sidecar_source_count"] = len(xmp_sources)
    for xmp in xmp_sources:
        key = normalized_sidecar_key(xmp)
        matches = sorted(images_by_key.get(key, []), key=lambda row: str(row["source_content_id"]))
        if not matches:
            stats["xmp_unmatched_sidecar_count"] += 1
            audit.append({
                "audit_type": "xmp_sidecar", "selection_status": "unmatched_sidecar",
                "xmp_source_content_id": xmp["source_content_id"],
                "xmp_relative_path": xmp["relative_path"],
                "normalized_parent_directory": key[0], "normalized_stem": key[1],
            })
            continue
        if len(matches) > 1:
            stats["xmp_multi_image_match_group_count"] += 1
        for image_source in matches:
            image_source_id = str(image_source["source_content_id"])
            matched_image_sources.add(image_source_id)
            mappings = mappings_by_source.get(image_source_id, [])
            if not mappings:
                xmp_missing_visual_sources.add(image_source_id)
            for mapping in mappings:
                canonical_id = str(mapping.get("canonical_visual_unit_id") or "")
                detail = {
                    "audit_type": "xmp_sidecar", "image_source_content_id": image_source_id,
                    "image_relative_path": image_source["relative_path"],
                    "xmp_source_content_id": xmp["source_content_id"],
                    "xmp_relative_path": xmp["relative_path"],
                    "normalized_parent_directory": key[0], "normalized_stem": key[1],
                    "tagged_original_visual_unit_id": mapping.get("visual_unit_id"),
                    "canonical_visual_unit_id": canonical_id,
                }
                if canonical_id not in image_by_id:
                    audit.append({**detail, "selection_status": "canonical_visual_unavailable"})
                    continue
                xmp_by_canonical[canonical_id].append(detail)
                audit.append({**detail, "selection_status": "mapped_canonical"})
    stats["xmp_matched_image_source_count"] = len(matched_image_sources)
    stats["xmp_canonical_unique_count"] = len(xmp_by_canonical)
    stats["xmp_missing_visual_count"] = len(xmp_missing_visual_sources)

    timelapse_rows = list(image_inputs["timelapse_rows"])
    stats["timelapse_input_row_count"] = len(timelapse_rows)
    ...
    stats["timelapse_sequence_count"] = len(by_sequence)
    mapped_position_count = 0
    canonical_position_keys: set[tuple[str, str, str]] = set()
    for sequence_id, members in sorted(by_sequence.items()):
        position_candidates: dict[str, list[tuple[str, dict[str, Any]]]] = defaultdict(list)
        for member in members:
            position = str(member.get("representative_position") or "").lower()
            if position not in {"first", "middle", "last"}:
                continue
            mapping = mapping_by_visual.get(str(member.get("visual_unit_id") or ""))
            canonical_id = str((mapping or {}).get("canonical_visual_unit_id") or "")
            mapped_position_count += int(bool(canonical_id))
            if canonical_id:
                canonical_position_keys.add((sequence_id, position, canonical_id))
            frame = image_by_id.get(canonical_id)
            if frame is None or frame.get("signature_status") != "PASS":
                audit.append({
                    "audit_type": "timelapse", "sequence_id": sequence_id,
                    "representative_position": position,
                    "tagged_original_visual_unit_id": member.get("visual_unit_id"),
                    "canonical_visual_unit_id": canonical_id,
                    "selection_status": "missing_or_unreadable_canonical_preview",
                })
                continue
            position_candidates[position].append((canonical_id, frame))
        chosen_positions: dict[str, dict[str, Any]] = {}
        used_canonical: set[str] = set()
        for position in ("middle", "first", "last"):
            candidates = sorted(position_candidates.get(position, []), key=lambda item: item[0])
            for canonical_id, frame in candidates:
                if canonical_id not in used_canonical:
                    chosen_positions[position] = frame
                    used_canonical.add(canonical_id)
                    break
        selected_positions, change_class, pair_evidence = select_timelapse_positions(chosen_positions, config)
        count_name = {1: "one", 2: "two", 3: "three"}.get(len(selected_positions), "zero")
        stats[f"timelapse_select_{count_name}_sequence_count"] += 1
        audit.append({
            "audit_type": "timelapse_sequence", "sequence_id": sequence_id,
            "selection_status": "selected", "timelapse_change_class": change_class,
            "selected_positions": selected_positions, "pair_similarity_evidence": pair_evidence,
        })
        for position in selected_positions:
            frame = chosen_positions[position]
            canonical_id = str(frame["visual_unit_id"])
            timelapse_by_canonical[canonical_id].append({
                "sequence_id": sequence_id, "representative_position": position,
                "timelapse_change_class": change_class,
                "pair_similarity_evidence": pair_evidence,
            })
    stats["timelapse_canonical_collapse_count"] = max(0, mapped_position_count - len(canonical_position_keys))
    stats["timelapse_selected_canonical_count"] = len(timelapse_by_canonical)

    finder_ids, xmp_ids, timelapse_ids = set(finder_by_canonical), set(xmp_by_canonical), set(timelapse_by_canonical)
    stats["finder_xmp_overlap_count"] = len(finder_ids & xmp_ids)
    stats["finder_timelapse_overlap_count"] = len(finder_ids & timelapse_ids)
    stats["xmp_timelapse_overlap_count"] = len(xmp_ids & timelapse_ids)
    stats["all_three_overlap_count"] = len(finder_ids & xmp_ids & timelapse_ids)
    selected_union = sorted(finder_ids | xmp_ids | timelapse_ids)
    q_rows: list[dict[str, Any]] = []
    for canonical_id in selected_union:
        frame = image_by_id[canonical_id]
        sources_hit = []
        if finder_by_canonical.get(canonical_id):
            sources_hit.append("finder_tag")
        if xmp_by_canonical.get(canonical_id):
            sources_hit.append("xmp_sidecar")
        if timelapse_by_canonical.get(canonical_id):
            sources_hit.append("timelapse")
        if frame.get("signature_status") != "PASS":
            stats["image_missing_or_unreadable_preview_count"] += 1
            audit.append({
                "audit_type": "final_image", "canonical_visual_unit_id": canonical_id,
                "selection_status": "excluded_missing_or_unreadable_preview",
                "selection_sources": sources_hit,
            })
            continue
        if finder_by_canonical.get(canonical_id):
            role, score = "image_finder_tag_seed", 100.0
        elif xmp_by_canonical.get(canonical_id):
            role, score = "image_xmp_sidecar_seed", 90.0
        else:
            role, score = "image_timelapse_representative", 80.0
        reasons = [f"image_selection_source:{name}" for name in sources_hit]
        for detail in finder_by_canonical.get(canonical_id, []):
            reasons.append(f"finder_tag:{detail.get('tag_id')}:{detail.get('tag_name') or detail.get('tag_raw')}")
        for detail in xmp_by_canonical.get(canonical_id, []):
            reasons.append(f"xmp_sidecar:{detail.get('xmp_source_content_id')}")
        for detail in timelapse_by_canonical.get(canonical_id, []):
            reasons.extend([
                f"sequence_id:{detail['sequence_id']}",
                f"representative_position:{detail['representative_position']}",
                f"timelapse_change_class:{detail['timelapse_change_class']}",
                "pair_similarity_evidence:" + json_text(detail["pair_similarity_evidence"]),
            ])
        ...
        q_rows.append(make_candidate(
            frame, queue_type="qwenvl_high_value", role=role, score=score,
            reasons=reasons, run_id=run_id, mode=mode, hashes=hashes,
            lineage=lineage, config=config,
        ))
        audit.append({
            "audit_type": "final_image", "canonical_visual_unit_id": canonical_id,
            "selection_status": "selected", "candidate_role": role,
            "selection_sources": sources_hit,
        })
    stats["canonical_image_input_count"] = len(image_by_id)
    stats["final_unique_image_qwen_count"] = len(q_rows)
    stats["image_unselected_count"] = max(0, len(image_by_id) - len(q_rows))
    stats["image_generic_visual_signal_candidate_count"] = 0
    summary_keys = [
        "canonical_image_input_count", "finder_tag_input_row_count", "finder_tag_unique_source_count",
        "finder_tag_image_source_count", "finder_tag_non_image_source_count", "finder_tag_missing_source_count",
        "finder_tag_mapped_visual_count", "finder_tag_canonical_unique_count", "finder_tag_canonical_collapse_count",
        "xmp_sidecar_source_count", "xmp_matched_image_source_count", "xmp_canonical_unique_count",
        "xmp_unmatched_sidecar_count", "xmp_multi_image_match_group_count", "xmp_missing_visual_count",
        "timelapse_input_row_count", "timelapse_sequence_count", "timelapse_select_one_sequence_count",
        "timelapse_select_two_sequence_count", "timelapse_select_three_sequence_count",
        "timelapse_selected_canonical_count", "timelapse_canonical_collapse_count",
        "finder_xmp_overlap_count", "finder_timelapse_overlap_count", "xmp_timelapse_overlap_count",
        "all_three_overlap_count", "final_unique_image_qwen_count", "image_unselected_count",
        "image_generic_visual_signal_candidate_count", "image_missing_or_unreadable_preview_count",
    ]
    summary = {key: stats[key] for key in summary_keys}
    return {"q_rows": q_rows, "audit": audit, "summary": summary, "stats": stats}
...
VIDEO_SEMANTIC_FIELDS = (
    "source_content_id", "visual_unit_id", "canonical_visual_unit_id",
    "candidate_role", "time_position_ms",
)


def video_semantic_set(rows: Iterable[Mapping[str, Any]]) -> set[tuple[Any, ...]]:
    return {
        tuple(row.get(field) for field in VIDEO_SEMANTIC_FIELDS)
        for row in rows if row.get("media_type") == "video"
    }


def compare_v24_non_screen_video_semantics(
    v25_rows: Sequence[Mapping[str, Any]], baseline_path: Path | None,
) -> dict[str, Any]:
    current = video_semantic_set(v25_rows)
    if baseline_path is None:
        return {
            "status": "NOT_REQUESTED", "video_logic_changed": False,
            "video_regression_ready": False, "v25_non_screen_video_count": len(current),
            "baseline_path": "", "baseline_non_screen_video_count": None,
            "excluded_rpreplay_source_count": None, "added_count": None, "removed_count": None,
        }
    baseline_rows = []
    with baseline_path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            try:
                ...
                raise RuntimeError(f"v24_video_baseline_jsonl_invalid:{line_number}:{exc}") from exc
    excluded_sources = {
        str(row.get("source_content_id") or "") for row in baseline_rows
        if row.get("media_type") == "video"
        and "rpreplay" in str(row.get("source_relative_path") or "").casefold()
    }
    baseline = video_semantic_set(
        row for row in baseline_rows
        if str(row.get("source_content_id") or "") not in excluded_sources
    )
    current = {
        row for row in current if str(row[0]) not in excluded_sources
    }
    added = sorted(current - baseline)
    removed = sorted(baseline - current)
    status = "PASS" if not added and not removed else "FAIL"
    return {
        "status": status, "video_logic_changed": status != "PASS",
        "video_regression_ready": True, "baseline_path": str(baseline_path),
        "semantic_fields": list(VIDEO_SEMANTIC_FIELDS),
        "baseline_non_screen_video_count": len(baseline),
        "v25_non_screen_video_count": len(current),
        "excluded_rpreplay_source_count": len(excluded_sources),
        "added_count": len(added), "removed_count": len(removed),
        "added_sample": [dict(zip(VIDEO_SEMANTIC_FIELDS, row)) for row in added[:20]],
        "removed_sample": [dict(zip(VIDEO_SEMANTIC_FIELDS, row)) for row in removed[:20]],
    }
...
def build_summary(
    ...
    video_regression = result.get("video_regression") or {"status": "NOT_REQUESTED"}
    gates = {
        ...
        "grid_executed": (
            signature_stats["grid_signature_success_count"] > 0
            and signature_stats.get("video_grid_signature_failed_count", signature_stats["grid_signature_failed_count"]) == 0
        ),
        ...
        "image_generic_visual_signal_candidate_zero": (
            stats["image_generic_visual_signal_candidate_count"] == 0
            and all(row.get("candidate_role") != "image_generic_visual_signal_candidate" for row in q_rows)
        ),
    }
    if video_regression.get("status") != "NOT_REQUESTED":
        gates["v24_non_screen_video_semantic_regression_pass"] = video_regression.get("status") == "PASS"
    if mode == "dry-run":
    ...
    return {
        ...
        "policy_status": pre["config"]["policy_status"],
        "commit_status": "DO_NOT_COMMIT" if mode == "dry-run" else "PENDING_COMMIT",
        "policy_reason_codes": ["v25_frozen_candidate_pending_finalizer"] if pre["config"]["policy_status"] == "FROZEN_CANDIDATE" else [],
        "policy_version": POLICY_VERSION, "script_version": SCRIPT_VERSION,
        ...
        **signature_stats, **vector_stats, **label_stats,
        ...
        "video_logic_changed": bool(video_regression.get("video_logic_changed")),
        "video_regression_ready": bool(video_regression.get("video_regression_ready")),
        "non_screen_video_regression_status": video_regression.get("status"),
        "non_screen_video_added_count": video_regression.get("added_count"),
        "non_screen_video_removed_count": video_regression.get("removed_count"),
        "excluded_rpreplay_source_count": video_regression.get("excluded_rpreplay_source_count"),
        **result.get("image_selection_summary", {}),
...
def write_outputs(out: Path, result: dict[str, Any], summary: dict[str, Any], reverse_rows: list[dict[str, Any]]) -> dict[str, str]:
    ...
    paths = {
        ...
        "image_selection_audit_jsonl": reports / "image_selection_audit.jsonl",
        "image_selection_summary_json": reports / "image_selection_summary.json",
        "video_v24_non_screen_semantic_regression_json": reports / "video_v24_non_screen_semantic_regression.json",
    ...
    write_jsonl(paths["image_selection_audit_jsonl"], result["image_selection_audit"])
    write_json(paths["image_selection_summary_json"], result["image_selection_summary"])
    write_json(paths["video_v24_non_screen_semantic_regression_json"], result["video_regression"])
...
def missing_candidate_columns(con: sqlite3.Connection) -> list[str]:
    existing = table_columns(con, "stop03_2_candidate_queue_items")
    return sorted(name for name in V25_DB_COLUMNS if name not in existing)


def validate_candidate_columns(con: sqlite3.Connection) -> None:
    missing = missing_candidate_columns(con)
    if missing:
        raise RuntimeError("blocked_v25_candidate_schema_migration_required:" + ",".join(missing))
...
def commit_candidates(
    ...
    if summary["policy_status"] != "FROZEN":
    ...
    try:
        ...
        validate_candidate_columns(con)
        ...
        columns = [
            ...
            "run_id", "script_version", "created_at", *V25_DB_COLUMNS,
        ]
        ...
        con.execute(
            ...
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                summary["run_id"], STAGE, "rule_based_v25_no_model", "", SCRIPT_VERSION,
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone Stop03-2 V25 generic candidate queues")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    baseline_path = (
        Path(args.video_regression_baseline).expanduser().resolve(strict=True)
        if args.video_regression_baseline else None
    )
    try:
        pre = preflight(db, out, config_path)
        pre["video_regression_baseline_path"] = str(baseline_path or "")
        pre["video_regression_ready"] = baseline_path is not None
        ...
        if pre["technical_status"] != "PASS":
            raise RuntimeError("v25_preflight_failed")
        mode = args.mode
        ...
        try:
            ...
            video_signature_failures = sum(
                1 for row in canonical_rows
                if row.get("signature_status") != "PASS" and row["media_type"] == "video"
            )
            image_signature_failures = sum(
                1 for row in canonical_rows
                if row.get("signature_status") != "PASS" and row["media_type"] == "image"
            )
            signature_stats["video_grid_signature_failed_count"] = video_signature_failures
            signature_stats["image_grid_signature_failed_count"] = image_signature_failures
            if video_signature_failures:
                raise RuntimeError("v25_video_derived_signature_failures")
            ...
            image_inputs = load_v25_image_inputs(con)
        finally:
            con.close()
        ...
        result = select_candidates(canonical_rows, raw_by_source, config, run_id, mode, hashes, lineage, image_inputs)
        result_repeat = select_candidates(canonical_rows, raw_by_source, config, run_id, mode, hashes, lineage, image_inputs)
        result["video_regression"] = compare_v24_non_screen_video_semantics(result["q_rows"], baseline_path)
        result_repeat["video_regression"] = compare_v24_non_screen_video_semantics(result_repeat["q_rows"], baseline_path)
        ...
        if summary["technical_status"] != "PASS":
            raise RuntimeError("v25_candidate_technical_gates_failed:" + json_text(summary["automatic_acceptance_gates"]))
        ...
        failure = {
            "technical_status": "FAIL", "policy_status": "FROZEN_CANDIDATE", "commit_status": "DO_NOT_COMMIT",
            "policy_version": POLICY_VERSION, "script_version": SCRIPT_VERSION,
