...
# -*- coding: utf-8 -*-
"""
Stop03-2 V12 video frame contact sheet generator.

Purpose:
- Read central SQLite DB and V12 candidate queue CSV files.
...
DEFAULT_V12_OUT = TEST_OUTPUT_ROOT / "stop03-2-candidate-queues-db-safe-v12_0_20260709_213500_full"
DEFAULT_OUT_DIR = TEST_OUTPUT_ROOT / "stop03-2-v12-video-frame-contact-sheet"
SCRIPT_VERSION = "stop03_2_v12_video_frame_contact_sheet_20260709_214000"
...
def main() -> int:
    ...
    ap.add_argument("--v12-out", type=Path, default=DEFAULT_V12_OUT)
    ap.add_argument("--out-dir", type=Path, default=DEFAULT_OUT_DIR)
    ap.add_argument("--max-img-width", type=int, default=150)
    args = ap.parse_args()

    db = args.db
    v12_out = args.v12_out
    out_dir = args.out_dir
    out_dir.mkdir(parents=True, exist_ok=True)

    qwen_csv = v12_out / "manifests" / "qwenvl_high_value_candidate_queue.csv"
    ocr_csv = v12_out / "manifests" / "ocr_trigger_candidate_queue.csv"
    ...
    summary = {
        ...
        "output_html": str(out_dir / "v12_video_frame_contact_sheet.html"),
        "output_group_summary_csv": str(out_dir / "v12_video_group_summary.csv"),
    }

    with (out_dir / "v12_video_group_summary.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(group_summaries[0].keys()) if group_summaries else [])
        writer.writeheader()
        writer.writerows(group_summaries)
    (out_dir / "v12_video_contact_sheet_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    parts.append("<title>Stop03-2 V12 Video Frame Contact Sheet</title>")
    parts.append(f"<style>{css}</style></head><body>")
    parts.append("<h1>Stop03-2 V12 视频帧总览</h1>")
    parts.append(f"<div class='meta'>DB: {html.escape(str(db))}<br>V12 OUT: {html.escape(str(v12_out))}<br>视频组: {summary['video_group_count']}；全部视频帧: {summary['video_frame_count']}；高价值帧: {summary['high_value_video_frame_count']}；OCR帧: {summary['ocr_video_frame_count']}；高价值+OCR重合: {summary['both_high_and_ocr_count']}</div>")
    ...
    html_path = out_dir / "v12_video_frame_contact_sheet.html"
