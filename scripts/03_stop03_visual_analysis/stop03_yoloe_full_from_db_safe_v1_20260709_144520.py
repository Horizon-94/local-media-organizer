...
# -*- coding: utf-8 -*-
"""
Stop03 YOLOE full from DB, local-only safe v1.
...
SCRIPT_VERSION = "stop03_yoloe_full_from_db_safe_v1_20260709_144520"
...
DEFAULT_OUT = Path("/Users/yourname/Documents/AI-Local/test-output/stop03-yoloe-full-db-safe-v1_20260709_144520")
...
def insert_visual_label(con: sqlite3.Connection, *, run_id: str, row: Dict[str, Any], det: Dict[str, Any], model_path: Path, mobileclip_path: Path) -> bool:
    ...
    label_id = "vl_" + sha256_text(json.dumps([row.get("visual_unit_id"), run_id, det], ensure_ascii=False, sort_keys=True))[:24]
    bbox_json = json.dumps(bbox, ensure_ascii=False)
    det_json = json.dumps(det, ensure_ascii=False, sort_keys=True)
    data = {
        "visual_label_id": label_id,
        "label_id": label_id,
        "id": label_id,
...
def db_audit(db: Path, model: Path, mobileclip: Path, registry: Optional[Path]) -> Dict[str, Any]:
    ...
    counts = {t: count_table(con, t) for t in ["source_assets", "derived_assets", "visual_units", "visual_labels", "embeddings", "model_runs", "processing_errors"]}
    schema = {t: table_columns(con, t) for t in ["visual_units", "derived_assets", "visual_labels", "model_runs"]}
    sample = []
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--registry", default="", help="Optional YOLOE prompt registry JSON")
...
def main() -> int:
    ...
    summary = {
        ...
        "outputs": {"result_csv": str(result_csv), "label_distribution_csv": str(label_csv)},
    }
    write_json(out / "reports" / "stop03_yoloe_full_summary.json", summary)
    print(json.dumps(summary, ensure_ascii=False, indent=2))
