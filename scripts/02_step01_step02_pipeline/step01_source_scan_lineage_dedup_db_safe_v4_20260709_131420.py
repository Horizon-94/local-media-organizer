...
SCRIPT_VERSION = "step01_source_scan_lineage_dedup_db_safe_v4_20260709_131420"
...
SOURCE_ROOT_GUARD = Path("/Users/yourname/Documents/MEDIA_ARCHIVE_TEST_SOURCE")
...
def parse_args():
    ...
    p.add_argument("sources", nargs="*", help="One or more source roots.")
    p.add_argument("--out", default=env_out, help="Base workspace output directory.")
    ...
    if not args.sources:
        raise SystemExit("错误：未指定素材源目录。用法：python3 step01_source_scan_lineage_dedup.py --out /path/workspace /path/source")
    if not args.out:
        first = Path(args.sources[0]).expanduser()
        args.out = str(first.parent / f"{first.name}-step01-workspace")
    return args
