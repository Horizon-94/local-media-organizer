...
# 1. 固定参数
# ============================================================
SCRIPT_VERSION = "step02_video_frame_c4s_id_from_db_safe_v3_20260709_141015"
...
def write_step02_video_frames_to_project_db(db_path: Path, run_id: str, video_reports, frame_rows_all):
    ...
    orphans_before_current_stage = count_orphan_visual_units(conn, current_stage_only=True)
    ...
    for r in frame_rows_all:
        try:
            ...
            # Hard guard: visual_units must never point to a missing derived_assets row.
            derived_exists = int(project_db_scalar(
                conn,
                "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?",
                (derived_id,),
            ) or 0)
            if derived_exists != 1:
            ...
            # Re-check after visual insert. Some old/unknown DB schemas may have triggers or constraints.
            derived_still_exists = int(project_db_scalar(
                conn,
                "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?",
                (derived_id,),
            ) or 0)
            if derived_still_exists != 1:
                ...
                row_errors += 1
                # Remove only the visual row created by this run to avoid a new orphan.
                try:
    ...
    actual_derived_rows_for_run = count_rows_by_model_run(conn, "derived_assets", run_id)
    actual_visual_rows_for_run = count_rows_by_model_run(conn, "visual_units", run_id)
    orphan_visual_units_for_run = count_orphan_visual_units(conn, run_id=run_id)
    orphans_after_current_stage = count_orphan_visual_units(conn, current_stage_only=True)

    integrity_errors = 0
    if actual_derived_rows_for_run != actual_visual_rows_for_run:
        integrity_errors += 1
    if orphan_visual_units_for_run != 0:
        integrity_errors += 1
    ...
    if actual_derived_rows_for_run != actual_visual_rows_for_run:
        error_parts.append(f"run_derived_visual_count_mismatch={actual_derived_rows_for_run}:{actual_visual_rows_for_run}")
    if orphan_visual_units_for_run:
        error_parts.append(f"run_orphan_visual_units={orphan_visual_units_for_run}")
    if first_frame_rows_seen != first_frame_rows_written:
    ...
    return {
        ...
        "run_id": run_id,
        "status": status,
        "derived_assets_upserted": actual_derived_rows_for_run,
        "visual_units_upserted": actual_visual_rows_for_run,
        ...
        "orphan_visual_units_for_this_run": orphan_visual_units_for_run,
        "orphan_visual_units_current_stage_before": orphans_before_current_stage,
        "orphan_visual_units_current_stage_after": orphans_after_current_stage,
