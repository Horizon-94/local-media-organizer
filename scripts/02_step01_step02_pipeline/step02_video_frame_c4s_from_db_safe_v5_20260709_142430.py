...
# 1. 固定参数
# ============================================================
SCRIPT_VERSION = "step02_video_frame_c4s_id_from_db_safe_v5_20260709_142430"
...
def write_step02_video_frames_to_project_db(db_path: Path, run_id: str, video_reports, frame_rows_all):
    ...
    for r in frame_rows_all:
        try:
            ...
            insert_or_replace_partial_dynamic(conn, "derived_assets", derived_values)
            conn.commit()

            derived_exists = int(project_db_scalar(conn, "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?", (derived_id,)) or 0)
            if derived_exists != 1:
                # One direct retry with the same partial row. If this still fails, do not write visual_units.
                insert_or_replace_partial_dynamic(conn, "derived_assets", derived_values)
                conn.commit()
                derived_exists = int(project_db_scalar(conn, "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?", (derived_id,)) or 0)
            if derived_exists != 1:
