...
LOCAL_FFMPEG = "ffmpeg"
LOCAL_FFPROBE = "ffprobe"
LOCAL_SIPS = "sips"
MODEL_USAGE_POLICY = "not_used_by_step02_video_frame_c4s_local_ffmpeg_ffprobe"
...
def _asset_info(path: Path, *, include_realpath: bool = False):
...
def _module_check(name: str, optional: bool = False):
    try:
        mod = __import__(name)
        version = getattr(mod, "__version__", "stdlib")
        return {"ok": True, "optional": optional, "version": str(version), "error": ""}
    except Exception as exc:
        return {"ok": False, "optional": optional, "version": "", "error": str(exc)}


def _tool_check(name: str, required: bool = True):
    rc, out, err = run_cmd(["bash", "-lc", f"command -v {name}"], timeout=5)
    ...
    if path:
        if name in ("ffmpeg", "ffprobe"):
            rc2, out2, err2 = run_cmd([path, "-version"], timeout=10)
            text = (out2 or err2 or "").splitlines()
            version = text[0][:240] if text else ""
            if rc2 != 0:
                error = (err2 or out2 or "")[:500]
        elif name == "sips":
            rc2, out2, err2 = run_cmd([path, "--version"], timeout=10)
            version = ((out2 or err2 or "").splitlines() or [""])[0][:240]
        else:
            version = path
    else:
        error = "not_found"
    return {"ok": bool(path), "required": required, "path": path, "version_or_help": version, "error": error}
...
def runtime_preflight(args=None, *, db_path: Path | None = None, out_path: Path | None = None):
    apply_offline_env()
    db_path = Path(db_path) if db_path else (Path(getattr(args, "db", "")) if args and getattr(args, "db", None) else DEFAULT_DB)
    out_path = Path(out_path) if out_path else (Path(getattr(args, "out", "")) if args and getattr(args, "out", None) else DEFAULT_OUT)
    expected_realpath = str(EXPECTED_PYTHON.resolve()) if EXPECTED_PYTHON.exists() else ""
    python_executable = sys.executable
    python_realpath = str(Path(sys.executable).resolve())
    bundled_app_python = (
        "/Contents/Frameworks/PipelinePython" in python_realpath
        and "/Python.framework/Versions/" in python_realpath
    )
    expected_match = (
        python_executable == str(EXPECTED_PYTHON)
        or bool(expected_realpath and python_realpath == expected_realpath)
        or bundled_app_python
    )

    dependencies = {
        "sqlite3": _module_check("sqlite3"),
        "csv": _module_check("csv"),
        "json": _module_check("json"),
        "hashlib": _module_check("hashlib"),
        "pathlib": _module_check("pathlib"),
        "subprocess": _module_check("subprocess"),
        "platform": _module_check("platform"),
    }
    missing_required = [k for k, v in dependencies.items() if not v.get("optional") and not v.get("ok")]
    local_tools = {
        "ffmpeg": _tool_check("ffmpeg", required=True),
        "ffprobe": _tool_check("ffprobe", required=True),
        "sips": _tool_check("sips", required=True),
    }

    assets = {
        "project_root": _asset_info(PROJECT_ROOT),
        "test_output_root": _asset_info(TEST_OUTPUT_ROOT),
        "current_test_source_root_read_protected": _asset_info(CURRENT_TEST_SOURCE_ROOT),
        "legacy_source_root_read_protected": _asset_info(LEGACY_SOURCE_ROOT),
        "expected_python_launcher": _asset_info(EXPECTED_PYTHON, include_realpath=True),
        "expected_python_realpath": _asset_info(Path(expected_realpath) if expected_realpath else EXPECTED_PYTHON),
        "default_db": _asset_info(DEFAULT_DB),
        "db": _asset_info(db_path),
        "default_out": _asset_info(DEFAULT_OUT),
        "output_base_parent": _asset_info(out_path.expanduser().resolve().parent if out_path else TEST_OUTPUT_ROOT),
    }
    if args and getattr(args, "step01_workspace", None):
        sw = Path(args.step01_workspace).expanduser()
        assets["step01_workspace"] = _asset_info(sw)
        assets["step01_video_queue"] = _asset_info(sw / "queues" / "process_queue_video.jsonl")

    blockers = []
    if not expected_match:
        blockers.append(f"python_mismatch: expected {EXPECTED_PYTHON}, got {python_executable} realpath={python_realpath}")
    if missing_required:
        blockers.append("missing_required_dependencies: " + ",".join(missing_required))
    for name, info in local_tools.items():
        if info.get("required") and not info.get("ok"):
            blockers.append(f"missing_required_local_tool: {name}")
    if db_path and not db_path.exists():
        blockers.append(f"db_missing: {db_path}")
    if out_path and not (_is_under(out_path, TEST_OUTPUT_ROOT) or _is_under(out_path, PROJECT_ROOT)):
        blockers.append(f"output_path_outside_allowed_roots: {out_path}")
    if args and getattr(args, "step01_workspace", None) and not (Path(args.step01_workspace) / "queues" / "process_queue_video.jsonl").exists():
        blockers.append(f"legacy_step01_video_queue_missing: {Path(args.step01_workspace) / 'queues' / 'process_queue_video.jsonl'}")
    ...
    return {
        ...
        "expected_python_realpath": expected_realpath,
        "expected_python_match": bool(expected_match),
        "bundled_app_python": bundled_app_python,
        "expected_script_local": str(PROJECT_ROOT / "scripts/02_step01_step02_pipeline/step02_video_frame_c4s_from_db_safe_v7_20260709_183800.py"),
        "project_root": str(PROJECT_ROOT),
        "test_output_root": str(TEST_OUTPUT_ROOT),
        "default_db": str(DEFAULT_DB),
        "default_out": str(DEFAULT_OUT),
        "input_selection_policy": "default_db_source_assets; legacy_step01_video_queue_selectable_by_cli_step01_workspace",
        ...
        "local_tool_policy": "ffmpeg_required_ffprobe_required_sips_required_local_system_tools_only",
        "source_media_policy": SOURCE_MEDIA_POLICY,
        "derived_write_policy": DERIVED_WRITE_POLICY,
        "required_local_assets": {
            "project_root": str(PROJECT_ROOT),
            "test_output_root": str(TEST_OUTPUT_ROOT),
            "db": str(db_path),
            "output_base_parent": str(out_path.expanduser().resolve().parent if out_path else TEST_OUTPUT_ROOT),
            "current_test_source_root_read_protected": str(CURRENT_TEST_SOURCE_ROOT),
            "expected_python_launcher": str(EXPECTED_PYTHON),
            "expected_python_realpath": expected_realpath,
            "ffmpeg": local_tools["ffmpeg"].get("path") or "ffmpeg",
            "ffprobe": local_tools["ffprobe"].get("path") or "ffprobe",
            "sips": local_tools["sips"].get("path") or "sips",
        },
        "assets": assets,
        ...
        "safety": {
            "network": "blocked_by_offline_env_not_used_by_step02_video_frame_c4s",
            "download": "not_used",
            "dependency_install": "not_used",
            "source_media_read": "read_only_video_probe_decode_ffmpeg_ffprobe_sips_validate",
...
def insert_or_replace_dynamic(conn, table_name: str, values: dict):
    """Legacy full-row upsert kept for model_runs/min-schema compatibility."""
...
def insert_or_replace_partial_dynamic(conn, table_name: str, values: dict, required_defaults: dict | None = None):
    """Safer project-table upsert for derived_assets/visual_units.

    V2/V3 used a full-row dynamic insert and filled every unknown project column
    with an empty default. On older project DB schemas this can interact badly
    with existing uniqueness/default contracts. This function only writes columns
    the row actually owns, plus explicit required defaults. It then verifies the
    primary logical id by caller-side SELECT.
    """
    cols = db_columns(conn, table_name)
    if not cols:
        return False
    ...
    for k in cols:
        lk = k.lower()
        # Fill only common NOT NULL-ish bookkeeping columns if the project table has them.
        if k not in row and lk in {"created_at", "updated_at", "generated_at", "processed_at"}:
            row[k] = now
        elif k not in row and lk in {"stage"}:
            row[k] = PROJECT_DB_STAGE
        elif k not in row and lk in {"script_version"}:
            row[k] = SCRIPT_VERSION
        elif k not in row and lk in {"model_name"}:
            row[k] = PROJECT_DB_MODEL_NAME
        elif k not in row and lk in {"model_path"}:
            row[k] = PROJECT_DB_MODEL_PATH
        elif k not in row and lk in {"status"}:
            row[k] = "done"
        elif k not in row and (lk.endswith("_count") or lk in {"input_count", "output_count"}):
            row[k] = 0
        elif k not in row and lk in {"error_message", "message", "note", "notes", "description"}:
            row[k] = ""
    if not row:
        return False
...
def fetch_video_queue_from_project_db(db_path: Path):
    ...
    # Step01 v4 stores media_kind/support_status/dedup_role in source_file_records,
    # not in source_assets. Reading source_assets alone would incorrectly treat
    # .srt/.xml/.arw/etc. as videos. Prefer the Step01 record table, then
    # fall back to source_assets only for older DBs that really have media_kind.
    rows = []
    if db_table_exists(conn, "source_file_records"):
        sfr_cols = db_columns(conn, "source_file_records")
        if "media_kind" in sfr_cols:
            rows = conn.execute(
                """
                SELECT sfr.*
                FROM source_file_records sfr
                WHERE lower(COALESCE(sfr.media_kind,''))='video'
                  AND lower(COALESCE(sfr.support_status,''))='supported'
                  AND lower(COALESCE(sfr.next_action,'process'))='process'
                  AND lower(COALESCE(sfr.dedup_role,'')) IN ('canonical','')
                ORDER BY sfr.relative_path
                """
            ).fetchall()
    if not rows:
        ...
        if "media_kind" in cols:
            rows = conn.execute(
                "SELECT * FROM source_assets WHERE lower(COALESCE(media_kind,''))='video' ORDER BY relative_path"
            ).fetchall()
        else:
            rows = []
    ...
    for r in rows:
        ...
        if not source_path or not source_content_id:
            continue
        ...
        queue_rows.append({
            ...
            "media_kind": "video",
            "next_action": "process",
            "dedup_role": d.get("dedup_role") or "canonical",
        })
    conn.close()
    return queue_rows


def project_db_audit(db_path: Path, runtime_preflight_report=None):
    db_path = db_path.expanduser().resolve()
    conn = sqlite3.connect(str(db_path))
    conn.row_factory = sqlite3.Row
    counts = {}
    for t in ["source_assets", "source_file_records", "derived_assets", "visual_units", "embeddings", "model_runs"]:
    ...
    queue_rows = fetch_video_queue_from_project_db(db_path)
    videos = []
    for d in queue_rows[:5]:
        videos.append({
            "source_content_id": d.get("source_content_id") or "",
            "source_file_id": d.get("source_file_id") or "",
            "relative_path": d.get("source_relative_path") or "",
            "extension": Path(d.get("source_path") or "").suffix.lower(),
            "absolute_path": d.get("source_path") or "",
        })
    return {
        "script_version": SCRIPT_VERSION,
        "mode": "db_audit_only",
        "db_path": str(db_path),
        "counts": counts,
        "pending_video_source_assets": len(queue_rows),
        "sample_videos": videos,
        "video_queue_policy": "source_file_records.media_kind=video/support_status=supported/next_action=process/dedup_role=canonical",
        "runtime_preflight": runtime_preflight_report or {},
    }

def project_db_scalar(conn, sql: str, params=()):
...
def count_orphan_visual_units(conn, run_id: str = "", current_stage_only: bool = False) -> int:
    if not db_table_exists(conn, "visual_units") or not db_table_exists(conn, "derived_assets"):
        return 0
    vu_cols = set(db_columns(conn, "visual_units"))
    where = "vu.derived_id IS NOT NULL AND vu.derived_id <> '' AND da.derived_id IS NULL"
    params = []
    if run_id and "model_run_id" in vu_cols:
        where += " AND COALESCE(vu.model_run_id,'')=?"
        params.append(run_id)
    if current_stage_only and "stage" in vu_cols:
        where += " AND COALESCE(vu.stage,'')=?"
        params.append(PROJECT_DB_STAGE)
    return int(project_db_scalar(conn, f"""
        SELECT COUNT(*)
...
def count_rows_by_model_run(conn, table_name: str, run_id: str) -> int:
    if not db_table_exists(conn, table_name):
        return 0
    cols = set(db_columns(conn, table_name))
    if "model_run_id" in cols:
        return int(project_db_scalar(conn, f"SELECT COUNT(*) FROM {table_name} WHERE COALESCE(model_run_id,'')=?", (run_id,)) or 0)
    if "run_id" in cols:
        return int(project_db_scalar(conn, f"SELECT COUNT(*) FROM {table_name} WHERE COALESCE(run_id,'')=?", (run_id,)) or 0)
    return 0


def insert_partial_ignore_dynamic(conn, table_name: str, values: dict, required_defaults: dict | None = None):
    """Insert only owned columns with INSERT OR IGNORE.

    This is deliberately different from INSERT OR REPLACE. derived_assets has a
    UNIQUE(sha256) contract in the project DB. Two adjacent extracted frames may
    be byte-identical. REPLACE on UNIQUE(sha256) deletes the earlier derived row
    and creates an orphan visual_unit. IGNORE preserves the existing canonical
    content row so visual_units can point to it.
    """
    cols = db_columns(conn, table_name)
    if not cols:
        return 0
...
def resolve_or_insert_derived_asset(conn, derived_values: dict):
    """Return (canonical_derived_id, mode).

    The project DB currently enforces UNIQUE(sha256) on derived_assets. That is
    a content-dedup contract. Therefore duplicate byte-identical frames must not
    create a second derived_assets row. Instead, their visual_units should point
    to the existing canonical derived_id while preserving their own
    visual_units.source_content_id/time_position_ms/visual_file.
    """
    sha = derived_values.get("sha256") or ""
    derived_id = derived_values.get("derived_id") or ""
    derived_path = derived_values.get("derived_path") or ""

    if derived_id:
        row = conn.execute("SELECT derived_id FROM derived_assets WHERE derived_id=?", (derived_id,)).fetchone()
        if row:
            return row[0], "existing_derived_id"

    if sha:
        row = conn.execute("SELECT derived_id FROM derived_assets WHERE sha256=?", (sha,)).fetchone()
        if row:
            return row[0], "existing_sha256"

    if derived_path:
        row = conn.execute("SELECT derived_id FROM derived_assets WHERE derived_path=?", (derived_path,)).fetchone()
        if row:
            return row[0], "existing_derived_path"

    insert_partial_ignore_dynamic(conn, "derived_assets", derived_values)
    conn.commit()

    if derived_id:
        row = conn.execute("SELECT derived_id FROM derived_assets WHERE derived_id=?", (derived_id,)).fetchone()
        if row:
            return row[0], "inserted_derived_id"
    if sha:
        row = conn.execute("SELECT derived_id FROM derived_assets WHERE sha256=?", (sha,)).fetchone()
        if row:
            return row[0], "inserted_or_existing_sha256"
    if derived_path:
        row = conn.execute("SELECT derived_id FROM derived_assets WHERE derived_path=?", (derived_path,)).fetchone()
        if row:
            return row[0], "inserted_or_existing_derived_path"
...
def write_step02_video_frames_to_project_db(db_path: Path, run_id: str, video_reports, frame_rows_all):
    ...
    output_count = 0
    row_errors = 0
    derived_insert_missing_errors = 0
    visual_orphan_prevented_errors = 0
    first_frame_rows_seen = 0
    first_frame_rows_written = 0
    expected_derived_ids = []
    expected_visual_ids = []
    derived_values_by_id = {}
    visual_values_by_id = {}
    derived_reused_by_sha256_count = 0
    ...
    for r in frame_rows_all:
        try:
            ...
            expected_derived_ids.append(derived_id)
            expected_visual_ids.append(visual_unit_id)
            ...
            metadata_json = json.dumps({
                ...
                "parent_source_file_id": r.get("parent_source_file_id") or "",
            ...
            derived_values = {
                ...
                "source_content_id": source_content_id,
                "source_file_id": r.get("parent_source_file_id") or "",
                "derived_type": "video_frame_jpg1280",
                "derived_path": frame_file,
                "file_path": frame_file,
                "absolute_path": frame_file,
                "relative_path": r.get("frame_relative_path") or "",
                "width": width,
                "height": height,
                "frame_index": frame_index,
                "time_position_ms": time_ms if time_ms is not None else -1,
                "time_ms": time_ms,
            ...
            canonical_derived_id, derived_mode = resolve_or_insert_derived_asset(conn, derived_values)
            if not canonical_derived_id:
                derived_insert_missing_errors += 1
                row_errors += 1
                continue
            if canonical_derived_id != derived_id:
                if derived_mode in {"existing_sha256", "inserted_or_existing_sha256"}:
                    derived_reused_by_sha256_count += 1
                elif derived_mode in {"existing_derived_path", "inserted_or_existing_derived_path"}:
                    derived_reused_by_path_count += 1
            elif derived_mode.startswith("inserted"):
                derived_inserted_count += 1

            # From this point onward the visual unit must reference the canonical
            # derived row actually present in derived_assets. This is what prevents
            # UNIQUE(sha256) content dedup from producing orphan visual_units.
            derived_id = canonical_derived_id
            visual_unit_id = stable_project_id("vu", "video_frame", source_content_id, frame_id, derived_id)
            expected_derived_ids[-1] = derived_id
            expected_visual_ids[-1] = visual_unit_id
            derived_values_by_id[derived_id] = dict(derived_values)
            ...
            visual_values_by_id[visual_unit_id] = dict(visual_values)
            # Clean only stale broken visual_units for the exact same generated
            # frame file. This removes V2-V5 test orphans without touching any
            # valid record or original media.
            try:
                cur = conn.execute(
                    """
                    DELETE FROM visual_units
                    WHERE visual_file=?
                      AND derived_id IS NOT NULL
                      AND derived_id <> ''
                      AND NOT EXISTS (
                          SELECT 1 FROM derived_assets da
                          WHERE da.derived_id = visual_units.derived_id
                      )
                    """,
                    (frame_file,),
                )
                orphan_visual_units_cleaned_count += int(cur.rowcount or 0)
                conn.commit()
            except Exception:
                pass
            insert_or_replace_partial_dynamic(conn, "visual_units", visual_values)
            conn.commit()

            derived_still_exists = int(project_db_scalar(conn, "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?", (derived_id,)) or 0)
            if derived_still_exists != 1:
                visual_orphan_prevented_errors += 1
                row_errors += 1
                try:
                    conn.execute("DELETE FROM visual_units WHERE visual_unit_id=?", (visual_unit_id,))
                    conn.commit()
                except Exception:
                    pass
                continue
            ...
            output_count += 1
            if frame_index == 1:
                first_frame_rows_written += 1
        except Exception:
            row_errors += 1

    # Verify only the rows this invocation attempted, independent of whether the
    # project table has model_run_id/run_id columns. Existing V2 orphans must not
    # make a clean V4 run fail; they can be repaired/cleaned separately.
    unique_derived_ids = sorted(set(expected_derived_ids))
    unique_visual_ids = sorted(set(expected_visual_ids))
    expected_derived_count = len(unique_derived_ids)
    expected_visual_count = len(unique_visual_ids)
    actual_derived_for_expected = 0
    actual_visual_for_expected = 0
    run_orphan_for_expected = 0
    if unique_derived_ids:
        for did in unique_derived_ids:
            actual_derived_for_expected += int(project_db_scalar(conn, "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?", (did,)) or 0)
    if unique_visual_ids:
        for vid in unique_visual_ids:
            actual_visual_for_expected += int(project_db_scalar(conn, "SELECT COUNT(*) FROM visual_units WHERE visual_unit_id=?", (vid,)) or 0)
    for vid in unique_visual_ids:
        row = conn.execute("SELECT derived_id FROM visual_units WHERE visual_unit_id=?", (vid,)).fetchone()
        if not row:
            continue
        did = row[0]
        exists = int(project_db_scalar(conn, "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?", (did,)) or 0)
        if exists != 1:
            run_orphan_for_expected += 1

    # Final repair pass: some project DB schemas/triggers/defaults can cause the
    # first frame derived_assets row to disappear after the visual_units upsert.
    # Re-insert any missing derived rows from the exact rows this run produced,
    # then re-check. This does not delete or modify original media, and it only
    # repairs rows whose frame files were generated by this invocation.
    missing_after_first_check = []
    for did in unique_derived_ids:
        if int(project_db_scalar(conn, "SELECT COUNT(*) FROM derived_assets WHERE derived_id=?", (did,)) or 0) != 1:
            missing_after_first_check.append(did)
    final_repair_derived_count = 0
    for did in missing_after_first_check:
        vals = derived_values_by_id.get(did)
        if vals:
            insert_or_replace_partial_dynamic(conn, "derived_assets", vals)
            final_repair_derived_count += 1
    if final_repair_derived_count:
        conn.commit()

    # Recompute expected counts after final repair.
    actual_derived_for_expected = 0
    actual_visual_for_expected = 0
    run_orphan_for_expected = 0
    for did in unique_derived_ids:
    ...
    if expected_derived_count != actual_derived_for_expected:
        integrity_errors += 1
    if expected_visual_count != actual_visual_for_expected:
        integrity_errors += 1
    if run_orphan_for_expected != 0:
        integrity_errors += 1
    if first_frame_rows_seen != first_frame_rows_written:
        integrity_errors += 1

    status = "done" if row_errors == 0 and integrity_errors == 0 else "failed_db_integrity_check"
    error_parts = []
    if row_errors:
        error_parts.append(f"db_output_row_errors={row_errors}")
    if derived_insert_missing_errors:
        error_parts.append(f"derived_insert_missing_errors={derived_insert_missing_errors}")
    if visual_orphan_prevented_errors:
        error_parts.append(f"visual_orphan_prevented_errors={visual_orphan_prevented_errors}")
    if expected_derived_count != actual_derived_for_expected:
        error_parts.append(f"expected_derived_missing={expected_derived_count}:{actual_derived_for_expected}")
    if expected_visual_count != actual_visual_for_expected:
        error_parts.append(f"expected_visual_missing={expected_visual_count}:{actual_visual_for_expected}")
    if run_orphan_for_expected:
        error_parts.append(f"expected_orphan_visual_units={run_orphan_for_expected}")
    if first_frame_rows_seen != first_frame_rows_written:
    ...
    return {
        ...
        "run_id": run_id,
        "status": status,
        "derived_assets_upserted": actual_derived_for_expected,
        "visual_units_upserted": actual_visual_for_expected,
        "db_output_row_errors": row_errors,
        "derived_insert_missing_errors": derived_insert_missing_errors,
        "visual_orphan_prevented_errors": visual_orphan_prevented_errors,
        "derived_reused_by_sha256_count": derived_reused_by_sha256_count,
        "derived_reused_by_path_count": derived_reused_by_path_count,
        "derived_inserted_count": derived_inserted_count,
        "orphan_visual_units_cleaned_count": orphan_visual_units_cleaned_count,
        "orphan_visual_units_for_this_run": run_orphan_for_expected,
        "orphan_visual_units_global_before": orphan_global_before,
        "orphan_visual_units_global_after": orphan_global_after,
        "expected_derived_count": expected_derived_count,
        "actual_derived_for_expected": actual_derived_for_expected,
        "expected_visual_count": expected_visual_count,
        "actual_visual_for_expected": actual_visual_for_expected,
        "first_frame_rows_seen": first_frame_rows_seen,
        "first_frame_rows_written": first_frame_rows_written,
        "missing_derived_after_first_check": len(missing_after_first_check),
...
def parse_args():
    ...
    p.add_argument("--preflight-only", action="store_true", help="Only check fixed local runtime, dependencies, tools, DB/output safety; do not run ffmpeg.")
...
def main():
    ...
    if args.db is None and args.step01_workspace is None:
        args.db = DEFAULT_DB
    if args.out is None:
        args.out = DEFAULT_OUT

    preflight = runtime_preflight(args, db_path=args.db or DEFAULT_DB, out_path=args.out or DEFAULT_OUT)
    if args.preflight_only:
        ...
        return 0 if preflight["validation_status"] == "PASS" else 2
    if preflight["blockers"]:
        print(json.dumps({"validation_status": "BLOCKED", "runtime_preflight": preflight}, ensure_ascii=False, indent=2))
        return 2

    args.runtime_preflight = preflight
    ...
    if args.db_audit_only:
        if not args.db:
            args.db = DEFAULT_DB
        print(json.dumps(project_db_audit(args.db, preflight), ensure_ascii=False, indent=2))
        return 0
    ...
    input_mode = "db_source_assets" if args.db else "legacy_step01_queue"
