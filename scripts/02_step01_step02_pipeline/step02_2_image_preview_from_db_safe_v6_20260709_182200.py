...
V6 addition:
- Preserve macOS Finder tags from Step01/source_finder_tags as manual high-value seed signals.
- If a Finder-tagged image ever falls inside a timelapse member set, it is not allowed to be silently skipped; a preview/visual_unit is generated for downstream high-value selection.
- Emit manual_high_value_image_seed manifests and optional DB table rows for downstream candidate selection.
"""
...
SCRIPT_VERSION = "step02_2_image_preview_from_db_safe_v6_20260709_182200"
...
def _is_bundled_app_python(path: Path) -> bool:
    """Recognize the signed desktop app's private, read-only Python runtime."""
    normalized = str(Path(path).expanduser().resolve(strict=False))
    return (
        "/Contents/Frameworks/PipelinePython" in normalized
        and "/Python.framework/Versions/" in normalized
        and normalized.endswith(("bin/python3", "bin/python3.12", "bin/python3.11", "bin/python3.10"))
    )
...
def runtime_preflight(db_path: Path, out_path: Path, step01_workspace: Path | None = None, input_mode: str = "db_source_assets") -> dict:
    ...
    bundled_app_python = (
        os.environ.get("MEDIA_ARCHIVE_PORTABLE_RUNTIME") == "1"
        and _is_bundled_app_python(actual_real)
    ) or _is_bundled_app_python(actual_real)
    compatible_python = actual_real == py_real or bundled_app_python
    blockers = []
    ...
    return {
        ...
        "python_compatibility_status": "compatible" if compatible_python else "incompatible",
...
def normalize_finder_tag_name(tag: Any) -> str:
    s = str(tag or "").strip()
    if not s:
        return ""
    if "\\u" in s or "\\U" in s:
        try:
            return s.encode("utf-8", errors="ignore").decode("unicode_escape")
        except Exception:
            return s
    return s


def finder_tags_json(tags: List[str]) -> str:
    cleaned = []
    seen = set()
    for t in tags or []:
        nt = normalize_finder_tag_name(t)
...
def finder_tags_by_source(conn: sqlite3.Connection) -> Dict[str, List[str]]:
    if "source_finder_tags" not in db_table_names(conn):
        return {}
    cols = set(db_columns(conn, "source_finder_tags"))
    if "source_content_id" not in cols or "tag_name" not in cols:
        return {}
    out: Dict[str, List[str]] = defaultdict(list)
    for r in conn.execute("SELECT source_content_id, tag_name FROM source_finder_tags"):
        sid = str(r[0] or "")
        tag = normalize_finder_tag_name(r[1])
        if sid and tag and tag not in out[sid]:
            out[sid].append(tag)
    return dict(out)
...
def ensure_project_db_tables(conn: sqlite3.Connection) -> None:
    ...
    """)
    conn.execute("""
    CREATE TABLE IF NOT EXISTS manual_high_value_visual_seeds (
        seed_id TEXT PRIMARY KEY,
        visual_unit_id TEXT NOT NULL,
        source_content_id TEXT NOT NULL,
        seed_source TEXT NOT NULL,
        seed_label TEXT NOT NULL,
        seed_strength TEXT NOT NULL,
...
def fetch_image_queue_rows_from_db(db_path: Path, limit: int = 0) -> List[dict]:
    ...
    ensure_project_db_tables(conn)
    tag_map = finder_tags_by_source(conn)
    sql = """
    ...
    for r in conn.execute(sql, params):
        ...
        tags = tag_map.get(d["source_content_id"], [])
        rows.append({
            ...
            "finder_tags": tags,
            "finder_tags_json": finder_tags_json(tags),
            "finder_tag_count": len(tags),
            "manual_high_value_seed": bool(tags),
            "manual_high_value_reason": "macos_finder_tag" if tags else "",
        })
    conn.close()
    return rows
...
def write_step02_image_outputs_to_project_db(db_path: Path, run_id: str, preview_rows: List[dict], visual_rows: List[dict]) -> dict:
    ...
    for r in preview_rows:
        ...
        visual_count += 1
        if (vu or {}).get("manual_high_value_seed") or r.get("manual_high_value_seed"):
            tags_json = (vu or {}).get("finder_tags_json") or r.get("finder_tags_json") or "[]"
            try:
                tags = json.loads(tags_json)
            except Exception:
                tags = []
            seed_label = ",".join(str(x) for x in tags)
            seed_id = "hvseed_" + sha256_text(f"finder_tag|{visual_unit_id}|{source_content_id}|{tags_json}", 24)
            conn.execute("""
                INSERT OR REPLACE INTO manual_high_value_visual_seeds
                (seed_id, visual_unit_id, source_content_id, seed_source, seed_label, seed_strength, candidate_type, reason, run_id, script_version)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (seed_id, visual_unit_id, source_content_id, "macos_finder_tag", seed_label, "strong_manual", "image_visual_unit", "Finder tag present on original image source", run_id, SCRIPT_VERSION))
            manual_seed_count += 1
    ...
    return {
        ...
        "manual_high_value_visual_seeds_upserted": manual_seed_count,
...
def audit_project_db_for_step02(db_path: Path, preflight: Optional[dict] = None) -> dict:
    ...
    """).fetchone()[0]
    if "source_finder_tags" in db_table_names(conn):
        out["finder_tagged_image_source_assets"] = conn.execute("""
            SELECT COUNT(DISTINCT sa.source_content_id)
            FROM source_finder_tags t JOIN source_assets sa ON sa.source_content_id=t.source_content_id
            WHERE sa.media_type='image' AND COALESCE(sa.online_status,1)=1 AND COALESCE(sa.is_deleted_or_missing,0)=0
        """).fetchone()[0]
    else:
        out["finder_tagged_image_source_assets"] = 0
...
def build_items(queue_rows: List[dict]) -> List[dict]:
    ...
    for r in queue_rows:
        ...
        raw_tags = r.get("finder_tags")
        if isinstance(raw_tags, str):
            try:
                parsed_tags = json.loads(raw_tags) if raw_tags.strip().startswith("[") else [raw_tags]
            except Exception:
            ...
            try:
                parsed_tags = json.loads(r.get("finder_tags_json") or "[]")
            except Exception:
                parsed_tags = []
        parsed_tags = [normalize_finder_tag_name(x) for x in parsed_tags if normalize_finder_tag_name(x)]
        items.append({
            **r,
            "source_path": str(p),
            ...
            "finder_tags": parsed_tags,
            "finder_tags_json": finder_tags_json(parsed_tags),
            "finder_tag_count": len(parsed_tags),
            "manual_high_value_seed": bool(parsed_tags),
            "manual_high_value_reason": "macos_finder_tag" if parsed_tags else "",
        })
    return items
...
def plan_jobs(items: List[dict], dirs: dict, seq_info: Dict[str, dict], member_ids: set, keyframe_ids: set) -> Tuple[List[dict], List[dict]]:
    ...
    for item in items:
        ...
        if sid in keyframe_ids:
            ...
            seq = seq_info[sid]
            if item.get("manual_high_value_seed"):
                preview_role = "manual_finder_tag_timelapse_member"
                should = True
                out_pool = dirs["keyframes"]
                sequence_id = seq["sequence_id"]
                rep = "manual_finder_tag"
                skipped_reason = ""
            else:
        ...
        d = {
            ...
            "finder_tags_json": item.get("finder_tags_json") or "[]",
            "finder_tag_count": item.get("finder_tag_count") or 0,
            "manual_high_value_seed": bool(item.get("manual_high_value_seed")),
            "manual_high_value_reason": item.get("manual_high_value_reason") or "",
        }
        decisions.append(d)
        if should:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for r in preview_rows:
        ...
        visual_rows.append({
            ...
            "finder_tags_json": r.get("finder_tags_json") or "[]",
            "finder_tag_count": r.get("finder_tag_count") or 0,
            "manual_high_value_seed": bool(r.get("manual_high_value_seed")),
            "manual_high_value_reason": r.get("manual_high_value_reason") or "",
            "manual_high_value_strength": "strong_manual_finder_tag" if r.get("manual_high_value_seed") else "",
        })

    manual_high_value_seed_rows = []
    for v in visual_rows:
        if not v.get("manual_high_value_seed"):
            continue
        try:
            tags = json.loads(v.get("finder_tags_json") or "[]")
        except Exception:
            tags = []
        manual_high_value_seed_rows.append({
            "seed_id": "hvseed_" + sha256_text(f"finder_tag|{v.get('visual_unit_id')}|{v.get('parent_source_content_id')}|{v.get('finder_tags_json')}", 24),
            "visual_unit_id": v.get("visual_unit_id"),
            "source_content_id": v.get("parent_source_content_id"),
            "source_relative_path": v.get("source_relative_path"),
            "visual_file": v.get("visual_file"),
            "candidate_type": "image_visual_unit",
            "seed_source": "macos_finder_tag",
            "seed_label": ",".join(str(x) for x in tags),
            "seed_strength": "strong_manual",
            "reason": "Finder tag present on original image source",
            "preview_role": v.get("preview_role"),
            "producer_step": "step02_2_image_preview",
            "producer_version": SCRIPT_VERSION,
    ...
    manifest_pref = [
        ...
        "source_relative_path", "extension", "finder_tags_json", "finder_tag_count", "manual_high_value_seed", "manual_high_value_reason",
        "preview_role", "backend", "task_action", "status", "output_path", "output_sha256", "width", "height", "validation_reason",
    ...
    manual_seed_pref = ["seed_id", "visual_unit_id", "source_content_id", "source_relative_path", "visual_file", "candidate_type", "seed_source", "seed_label", "seed_strength", "reason", "preview_role", "producer_step", "producer_version", "run_invocation_id"]
    write_csv(dirs["manifests"] / "manual_high_value_image_seed_manifest.csv", manual_high_value_seed_rows, manual_seed_pref)
    write_csv(dirs["manifests_history"] / f"{prefix}_manual_high_value_image_seed_manifest.csv", manual_high_value_seed_rows, manual_seed_pref)
    write_jsonl(dirs["manifests"] / "manual_high_value_image_seed_manifest.jsonl", manual_high_value_seed_rows)
    write_jsonl(dirs["manifests_history"] / f"{prefix}_manual_high_value_image_seed_manifest.jsonl", manual_high_value_seed_rows)
    ...
    summary = {
        ...
        "finder_tagged_input_image_count": sum(1 for i in items if i.get("manual_high_value_seed")),
        "finder_tagged_visual_unit_count": sum(1 for v in visual_rows if v.get("manual_high_value_seed")),
        "manual_high_value_seed_count": len(manual_high_value_seed_rows),
        "manual_high_value_seed_policy": "macOS Finder tags are strong manual high-value seeds; tagged timelapse members must keep visual_units",
        ...
        "manual_high_value_seed_manifest_csv": str(dirs["manifests"] / "manual_high_value_image_seed_manifest.csv"),
        "manual_high_value_seed_manifest_jsonl": str(dirs["manifests"] / "manual_high_value_image_seed_manifest.jsonl"),
        ...
        "downstream_ready_for": ["YOLOE", "Qwen-VL", "embedding", "high_value_candidate_selector"],
    }
