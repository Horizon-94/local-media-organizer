...
# 1. 固定参数
# ============================================================
SCRIPT_VERSION = "step02_video_frame_c4s_id_from_step01_queue_v4_short1500_history_telemetry_20260706"
...
def write_reports(
    ...
    summary = {
        ...
        "step01_video_queue": str(args.step01_workspace / "queues" / "process_queue_video.jsonl"),
        "output_dir": str(OUT),
...
def parse_args():
    p = argparse.ArgumentParser(description="Step02 C4S-ID-INC video frame extraction from Step01 video queue.")
    p.add_argument("--step01-workspace", required=True, type=Path, help="Step01 workspace directory.")
    p.add_argument("--out", required=True, type=Path, help="Step02 output directory. Reuse same dir for resume.")
    ...
    p.add_argument("--run-phase", choices=["auto", "first15", "resume", "manual"], default="auto", help="Report phase label. auto: first15 when --limit-new > 0 else resume.")
...
def main():
    ...
    args.run_phase_resolved = ("first15" if args.limit_new > 0 else "resume") if args.run_phase == "auto" else args.run_phase
