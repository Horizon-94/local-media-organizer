...
SCRIPT_VERSION = "step02_2_image_preview_from_step01_queue_v2_a9t3_interface_terminal_log_20260706"
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Step02-2 image preview from Step01 image queue, A9T-v3 compatible internals.")
    ap.add_argument("--step01-workspace", required=True)
    ...
    step01_workspace = Path(args.step01_workspace).expanduser().resolve()
    ...
    print(f"queue read done: queue_rows={len(queue_rows)} eligible_images={len(items)} elapsed={queue_elapsed:.3f}s", flush=True)
    ...
    summary = {
        ...
        "input_policy": "consume_step01_canonical_image_queue_only",
