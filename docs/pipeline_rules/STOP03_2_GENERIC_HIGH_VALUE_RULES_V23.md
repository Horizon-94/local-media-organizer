# Stop03-2 通用高价值帧与 OCR 候选规则 V23

状态：`FROZEN_CANDIDATE`
说明：本文件定义正式通用规则。完成 V23 代码实现、当前数据回归、数据库写入与幂等验证后，状态改为 `FROZEN`。
...
V23 不是“人工挑看起来好的帧”，而是一个确定性、可复跑、可解释、可写库的本地规则。
...
以下数量只属于当前测试数据，不属于 V23 规则，不得写成固定验收目标：
...
V23 不得依赖某个旧输出目录中的 V14、V17、V20 或 V22 manifest 才能运行。

V14 的高信号思想、V17 的时间覆盖思想和 V22 的 vector/grid 去重逻辑必须吸收进 V23 代码，由 V23 直接根据中心数据库现有 YOLOE、OpenCLIP、派生帧和时间码重新计算。
...
5. 若未来 Step02 采样间隔变化，V23 仍按 sampled-frame index 工作，同时记录实际时间间隔供审计。

### 4.3 anchor 不是最终帧

每个 anchor 只是覆盖位置。V23 必须在 anchor 附近的候选帧中进行确定性择优，不允许直接把 anchor 当作最终高价值帧。
...
V23 固定使用以下角色，不再把所有视频候选统称为“高价值帧”：
...
进入 V23 前，先使用中心 source/frame dedup 结果，只处理 canonical visual。
...
普通图片 OCR 不在本规则中自动扩大。延时摄影按单张代表图处理。
...
V23 必须先完成所有内存选择和一致性校验，再使用单事务写入。
...
V23 冻结后不得原地修改。任何算法、阈值语义、输入范围或角色变化必须创建 V24。
...
V22 仅作为历史开发记录，不是 V23 的运行或验收依赖；V23 不要求与 V22 候选数量或候选集合比较。
