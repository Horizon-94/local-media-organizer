# Stop03-2 通用高价值帧与 OCR 候选规则 V24

状态：`FROZEN_CANDIDATE`
说明：本文件定义正式通用规则。完成 V24 代码实现、当前数据回归、数据库写入与幂等验证后，状态改为 `FROZEN`。
适用范围：所有后续本地图片、视频、录屏、延时摄影素材批次。不得针对当前文件夹、项目主题或当前统计数量写专用规则。

V24 只修订图片候选规则。视频 coverage、tail-only remap、局部择优、refill、视频内部去重、高信号 supplement、普通视频 OCR、录屏分流和时间映射必须完整保持 V23 行为。

---

## 1. 规则目标

V24 不是“人工挑看起来好的帧”，而是一个确定性、可复跑、可解释、可写库的本地规则。
...
以下数量只属于当前测试数据，不属于 V24 规则，不得写成固定验收目标：
...
V24 不得依赖某个旧输出目录中的 V14、V17、V20 或 V22 manifest 才能运行。

V14 的高信号思想、V17 的时间覆盖思想和 V22 的 vector/grid 去重逻辑必须吸收进 V24 代码，由 V24 直接根据中心数据库现有 YOLOE、OpenCLIP、派生帧和时间码重新计算。
...
5. 若未来 Step02 采样间隔变化，V24 仍按 sampled-frame index 工作，同时记录实际时间间隔供审计。

### 4.3 anchor 不是最终帧

每个 anchor 只是覆盖位置。V24 必须在 anchor 附近的候选帧中进行确定性择优，不允许直接把 anchor 当作最终高价值帧。
...
V24 固定使用以下角色，不再把所有视频候选统称为“高价值帧”：
...
进入 V24 前，先使用中心 source/frame dedup 结果，只处理 canonical visual。
...
V24 禁止 `image_generic_visual_signal_candidate`。普通图片不得因 YOLOE、generic score、OpenCLIP 新颖度、grid、主体数量、构图或文字区域评分自动进入 Qwen。
...
V24 必须先完成所有内存选择和一致性校验，再使用单事务写入。
...
V24 冻结后不得原地修改。任何算法、阈值语义、输入范围或角色变化必须创建 V24。
...
V22 仅作为历史开发记录，不是 V24 的运行或验收依赖；V24 不要求与 V22 候选数量或候选集合比较。
